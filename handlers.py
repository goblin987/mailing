...
CTX_INVITE_DETAILS = "invite_details" # Store {'days': int, 'bots': int} - Not currently used post-creation
CTX_EXTEND_CODE = "extend_code"
CTX_ADD_BOTS_CODE = "add_bots_code"
CTX_FOLDER_ID = "folder_id"
CTX_FOLDER_NAME = "folder_name"
CTX_FOLDER_ACTION = "folder_action" # e.g., 'add', 'remove', 'rename' - Currently implicit
...
def clear_conversation_data(context: CallbackContext):
    ...
    # Adjusted keys to clear based on usage
    keys_to_clear = [
    ...
    # Always restore user_id and lang if they exist
...
def get_user_id_and_lang(update: Update, context: CallbackContext) -> tuple[int | None, str]:
    ...
    if user_id and not lang:
        ...
        lang = 'en' # Default if still no language found
        if user_id and context.user_data: context.user_data[CTX_LANG] = lang

    return user_id, lang

# Keep this async as it does the actual IO
async def _send_or_edit_message(update: Update, context: CallbackContext, text: str, **kwargs):
    ...
    message_id = None # Start with no message ID

    # Try to get message_id for editing
    query = update.callback_query # Store query object
    if query:
        message_id = query.message.message_id
        # Ensure user_id is from the callback initiator if chat_id isn't available
        if not chat_id: chat_id = query.from_user.id
    # Check context only if it exists and query doesn't provide the message
    elif context.user_data and 'message_id' in context.user_data:
        message_id = context.user_data.get(CTX_MESSAGE_ID)
        if not chat_id: chat_id = user_id # Fallback to user_id if no chat context

    # Final check for chat_id
    if not chat_id:
        log.error(f"Cannot determine chat_id for sending/editing message. User ID: {user_id}")
    ...
    sent_message = None # To store the result of send_message

    try:
        if query:
            # Answer callback query silently first
            try:
                # Check if already answered before answering again
                # Use query.id as a check against duplicate answers
                if not context.bot_data.get(f'answered_{query.id}', False):
                    await query.answer()
                    context.bot_data[f'answered_{query.id}'] = True # Mark as answered
                answered_callback = True
            except (BadRequest, TelegramError) as cb_e: # Catch PTB errors
                log.debug(f"Ignoring callback answer error: {cb_e}")

            # Use the message_id from the callback query's message
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=query.message.message_id, # Use message_id from query
                text=text,
                **kwargs
            )
        ...
        # Fallback: Try editing a stored message ID if no direct update source
             ...
             log.debug(f"Attempting edit using stored message_id: {message_id} in chat {chat_id}")
             await context.bot.edit_message_text(
                 chat_id=chat_id,
        ...
        if "message is not modified" in str(e).lower():
            ...
            # If callback wasn't answered before, try now
            if query and not answered_callback and not context.bot_data.get(f'answered_{query.id}', False):
                try:
                    await query.answer()
            ...
            log.warning(f"Failed to edit message for user {user_id} (maybe deleted or wrong ID: {message_id}): {e}. Sending new.")
            try:
                sent_message = await context.bot.send_message(chat_id=chat_id, text=text, **kwargs)
                if context.user_data: context.user_data[CTX_MESSAGE_ID] = sent_message.message_id
            ...
            if context.user_data: context.user_data.pop(CTX_MESSAGE_ID, None) # Clear invalid message ID
             ...
             log.warning(f"Failed to reply - original message likely deleted. Sending standalone msg. User: {user_id}, Error: {e}")
             try:
                 sent_message = await context.bot.send_message(chat_id=chat_id, text=text, **kwargs)
                 if context.user_data: context.user_data[CTX_MESSAGE_ID] = sent_message.message_id
             except Exception as send_e: log.error(f"Failed to send fallback standalone msg to {chat_id}: {send_e}")
        else:
         ...
         if isinstance(e, RetryAfter):
             log.warning(f"Flood control hit: Waiting {e.retry_after} seconds.")
             # Note: PTB doesn't automatically handle this, ideally add retry logic or inform user.
         try: await context.bot.send_message(chat_id=chat_id, text=get_text(user_id, 'error_generic', lang=lang), parse_mode=parse_mode)
...
async def error_handler(update: object, context: CallbackContext) -> None:
    ...
    if isinstance(update, Update) and update.effective_chat:
        ...
        try: error_message = get_text(user_id, 'error_generic', lang=lang)
        except Exception: pass
        await _send_or_edit_message(update, context, error_message) # Use internal helper
    # Avoid clearing data on errors that might be recoverable or unrelated to conversation state
...
def format_dt(timestamp: int | None, tz=LITHUANIA_TZ, fmt='%Y-%m-%d %H:%M') -> str:
    """Formats a Unix timestamp into a human-readable string in the specified timezone."""
    if not timestamp:
        # Get default text using a placeholder user_id=0 and lang='en' if context isn't available
        return get_text(0, 'task_value_not_set', lang='en')
    try:
...
def build_client_menu(user_id, context: CallbackContext):
    ...
    end_date = format_dt(sub_end_ts, fmt='%Y-%m-%d')
    ...
    if userbot_phones:
        for i, phone in enumerate(userbot_phones, 1):
            ...
            # Determine status icon based on Telethon status strings
            status_icon = "⚪️" # Default inactive
            if bot_db_info:
            ...
            # Use translation keys for the line and error if defined
            menu_text += get_text(user_id, 'client_menu_userbot_line', lang=lang,
                                  index=i, status_icon=status_icon, display_name=display_name,
                                  status=html.escape(status)) + "\n"

            if last_error:
                escaped_error = html.escape(last_error)
                error_line = get_text(user_id, 'client_menu_userbot_error', lang=lang,
                                      error=f"{escaped_error[:100]}{'...' if len(escaped_error)>100 else ''}")
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(get_text(user_id, 'client_menu_button_stats', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}view_stats")], # Corrected callback key
...
def build_admin_menu(user_id, context: CallbackContext):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(get_text(user_id, 'admin_button_view_logs', lang=lang), callback_data=f"{CALLBACK_ADMIN_PREFIX}view_logs?page=0")],
    ]
    markup = InlineKeyboardMarkup(keyboard)
...
def build_pagination_buttons(base_callback_data: str, current_page: int, total_items: int, items_per_page: int, lang: str = 'en') -> list:
    buttons = []
    if total_items <= items_per_page: return [] # No pagination needed if items fit on one page
    ...
    # Previous Button
    if current_page > 0:
        prev_text = get_text(0, 'pagination_prev', lang=lang) # Use lang
        row.append(InlineKeyboardButton(prev_text, callback_data=f"{base_callback_data}?page={current_page - 1}"))
    else: # Add a disabled placeholder if on first page? Optional.
        # row.append(InlineKeyboardButton(" ", callback_data=f"{CALLBACK_GENERIC_PREFIX}noop")) # Spacer
        pass

    # Page Indicator Button (Not Clickable)
    if total_pages > 1:
        page_text = get_text(0,'pagination_page',lang=lang).format(current=current_page + 1, total=total_pages)
        row.append(InlineKeyboardButton(page_text, callback_data=f"{CALLBACK_GENERIC_PREFIX}noop"))

    # Next Button
    if current_page < total_pages - 1:
        next_text = get_text(0, 'pagination_next', lang=lang) # Use lang
        row.append(InlineKeyboardButton(next_text, callback_data=f"{base_callback_data}?page={current_page + 1}"))
    else: # Add disabled placeholder? Optional.
         # row.append(InlineKeyboardButton(" ", callback_data=f"{CALLBACK_GENERIC_PREFIX}noop")) # Spacer
         pass
...
async def admin_command(update: Update, context: CallbackContext) -> str | int:
    ...
    return ConversationHandler.END # End conversation after showing menu
...
async def cancel_command(update: Update, context: CallbackContext) -> int:
    ...
    # Try to show the main menu if applicable after cancelling
    if is_admin(user_id):
        return await admin_command(update, context)
...
async def conversation_fallback(update: Update, context: CallbackContext) -> int:
     ...
     # Check for commands within a conversation
     ...
     # If not a recognized command, assume unexpected input
     await _send_or_edit_message(update, context, get_text(user_id, 'conversation_fallback', lang=lang))
     clear_conversation_data(context) # Clear data as state is lost
     return ConversationHandler.END

# --- Main Menu & Language ---
async def client_menu(update: Update, context: CallbackContext) -> int:
    user_id, lang = get_user_id_and_lang(update, context)
    # If called from a callback, clear volatile data first
    if update.callback_query:
        clear_conversation_data(context)
        lang = context.user_data.get(CTX_LANG, 'en') # Re-fetch lang after clearing
...
async def client_ask_select_language(update: Update, context: CallbackContext) -> int:
    ...
    for code, name in sorted_languages:
        ...
        # Simple layout: max 2 buttons per row
        if len(row) >= 2:
            buttons.append(row)
            row = []
    ...
    return ConversationHandler.END # Expect callback, end state here
...
async def set_language_handler(update: Update, context: CallbackContext) -> int:
    ...
    current_lang = context.user_data.get(CTX_LANG, 'en') # Lang before change for potential error msg

    async def answer_callback_task(text=None, show_alert=False):
        try:
             # Check if already answered using query ID stored in bot_data
             if not context.bot_data.get(f'answered_{query.id}', False):
    ...
    if lang_code not in language_names:
        await answer_callback_task(get_text(user_id, 'error_invalid_input', lang=current_lang), show_alert=True)
        return ConversationHandler.END
    ...
    if db.set_user_language(user_id, lang_code):
         ...
         await answer_callback_task() # Answer silently on success
    else:
        await answer_callback_task(get_text(user_id, 'language_set_error', lang=current_lang), show_alert=True)
...
async def process_admin_phone(update: Update, context: CallbackContext) -> str | int:
     ...
     # Simple regex for + followed by digits
     if not re.fullmatch(r'\+\d{9,15}', phone_raw):
         await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_invalid_phone', lang=lang))
         return STATE_WAITING_FOR_PHONE
     phone = phone_raw
     context.user_data[CTX_PHONE] = phone
     log.info(f"Admin {user_id} entered phone: {phone}")
     await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_prompt_api_id', lang=lang))
...
async def process_admin_api_hash(update: Update, context: CallbackContext) -> str | int:
    ...
    # Basic check for length and hex characters
    if not api_hash or len(api_hash) < 30 or not re.match('^[a-fA-F0-9]+$', api_hash):
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_invalid_api_hash', lang=lang))
        return STATE_WAITING_FOR_API_HASH
    ...
    if not phone or not api_id:
        await _send_or_edit_message(update, context, get_text(user_id, 'session_expired', lang=lang))
        clear_conversation_data(context)
        return ConversationHandler.END
    ...
    try:
        ...
        if auth_status == 'code_needed':
            ...
            await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_prompt_code', lang=lang, phone=html.escape(phone)))
            return STATE_WAITING_FOR_CODE_USERBOT
        elif auth_status == 'password_needed':
            context.user_data[CTX_AUTH_DATA] = auth_data
            await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_prompt_password', lang=lang, phone=html.escape(phone)))
            return STATE_WAITING_FOR_PASSWORD
        elif auth_status == 'already_authorized':
             log.warning(f"Userbot {phone} is already authorized (session likely exists).")
             # Ensure DB entry exists and is marked active
             ...
             telethon_api.get_userbot_runtime_info(phone) # This handles initialization
             await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_already_auth', lang=lang, display_name=html.escape(phone)))
             clear_conversation_data(context)
             return ConversationHandler.END
        else: # Handle errors from start_authentication_flow
            ...
            if "flood wait" in error_msg.lower():
                ...
                locals_for_format['seconds'] = re.search(r'\d+', error_msg).group(0) if re.search(r'\d+', error_msg) else '?'
            ...
            await _send_or_edit_message(update, context, get_text(user_id, key, lang=lang, **locals_for_format))
            clear_conversation_data(context)
            return ConversationHandler.END
    except Exception as e:
        log.error(f"Exception during start_authentication_flow for {phone}: {e}", exc_info=True)
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_error_unknown', lang=lang, phone=html.escape(phone), error=html.escape(str(e))))
        clear_conversation_data(context)
        return ConversationHandler.END
...
async def process_admin_userbot_code(update: Update, context: CallbackContext) -> str | int:
    ...
    api_id = context.user_data.get(CTX_API_ID) # Keep API info for DB save
    api_hash = context.user_data.get(CTX_API_HASH)
    ...
    if not auth_data:
        ...
        return ConversationHandler.END
    if not code.isdigit():
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_error_code_invalid', lang=lang, phone=html.escape(phone), error="Format incorrect"))
        return STATE_WAITING_FOR_CODE_USERBOT
    ...
    try:
        ...
        # Clear auth data regardless of outcome now
        context.user_data.pop(CTX_AUTH_DATA, None)

        if comp_status == 'success':
            phone_num = comp_data.get('phone', phone) # Use actual phone from result
            username = comp_data.get('username')
            display_name = f"@{username}" if username else phone_num
            # DB entry is handled inside complete_authentication_flow now
            ...
            telethon_api.get_userbot_runtime_info(phone_num) # Initialize runtime
            clear_conversation_data(context)
            return ConversationHandler.END
        elif comp_status == 'error' and "Password required" in comp_data.get('error_message','').lower():
            # This case shouldn't typically happen if start_auth worked, but handle it.
            await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_error_password_needed_unexpected', lang=lang))
            clear_conversation_data(context)
            return ConversationHandler.END
        else: # Handle other errors from complete_authentication_flow
...
async def process_admin_userbot_password(update: Update, context: CallbackContext) -> str | int:
    ...
    if not auth_data:
        ...
        return ConversationHandler.END
    if not password:
        await _send_or_edit_message(update, context, get_text(user_id, 'error_invalid_input', lang=lang))
        return STATE_WAITING_FOR_PASSWORD
    ...
    try:
        ...
        if comp_status == 'success':
            ...
            username = comp_data.get('username')
            display_name = f"@{username}" if username else phone_num
            # DB handled inside complete_auth now
            await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_add_success', lang=lang, display_name=html.escape(display_name)))
            telethon_api.get_userbot_runtime_info(phone_num) # Initialize runtime
            clear_conversation_data(context)
            return ConversationHandler.END
...
async def process_admin_invite_details(update: Update, context: CallbackContext) -> int:
    ...
    if not match:
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_invite_invalid_format', lang=lang))
        return STATE_WAITING_FOR_SUB_DETAILS

    try:
        days = int(match.group(1))
        bots_needed = int(match.group(2)) # Note: Bot count is informational for admin, not strictly enforced by code yet
        if days <= 0 or bots_needed <= 0: raise ValueError()
    except (ValueError, AssertionError):
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_invite_invalid_numbers', lang=lang))
        return STATE_WAITING_FOR_SUB_DETAILS

    await _send_or_edit_message(update, context, get_text(user_id, 'admin_invite_generating', lang=lang))
    code = str(uuid.uuid4().hex)[:8] # Generate 8-char hex code
    ...
    if db.create_invitation(code, sub_end_ts):
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_invite_success', lang=lang, code=code, end_date=end_date_str, count=bots_needed))
    else:
        db.log_event_db("Invite Gen Failed", f"Code: {code} (duplicate?)", user_id=user_id)
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_invite_db_error', lang=lang))
...
async def process_admin_extend_days(update: Update, context: CallbackContext) -> int:
    ...
    now_ts = int(datetime.now(UTC_TZ).timestamp())
    start_ts = max(now_ts, current_end_ts) # Extend from now if expired, otherwise from current end
    ...
    if db.extend_subscription(code, new_end_ts):
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_extend_success', lang=lang, code=html.escape(code), days=days_to_add, new_end_date=new_end_date_str))
    else:
        db.log_event_db("Sub Extend Failed", f"Code: {code}", user_id=user_id)
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_extend_db_error', lang=lang))
...
async def process_admin_add_bots_code(update: Update, context: CallbackContext) -> int:
    ...
    if not client:
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_assignbots_invalid_code', lang=lang))
        return STATE_WAITING_FOR_ADD_USERBOTS_CODE

    context.user_data[CTX_ADD_BOTS_CODE] = code
    # Fetch current bot count correctly even if client not activated yet
    current_bots = [b for b in db.get_all_userbots(assigned_status=True) if b['assigned_client'] == code]
    current_count = len(current_bots)

    await _send_or_edit_message(update, context, get_text(user_id, 'admin_assignbots_prompt_count', lang=lang, code=html.escape(code), current_count=current_count))
...
async def process_admin_add_bots_count(update: Update, context: CallbackContext) -> int:
    ...
    # Find available bots (unassigned AND active)
    available_bots = db.get_unassigned_userbots(limit=count_to_add)

    if len(available_bots) < count_to_add:
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_assignbots_no_bots_available', lang=lang, needed=count_to_add, available=len(available_bots)))
        # Don't clear data, let admin try again or cancel
        return STATE_WAITING_FOR_ADD_USERBOTS_COUNT

    # Assign the found bots
    ...
    if success:
        ...
        assigned_count = len(available_bots) # How many we attempted to assign

        # Use the message from db.assign_userbots_to_client for detailed feedback
        if "Successfully assigned" in message:
             # Try to extract counts for better message formatting
             assigned_match = re.search(r"Successfully assigned (\d+)", message)
             actually_assigned = int(assigned_match.group(1)) if assigned_match else assigned_count

             if actually_assigned == assigned_count and "Failed:" not in message: # Full success
                 final_message = get_text(user_id, 'admin_assignbots_success', lang=lang, count=assigned_count, code=html.escape(code))
             else: # Partial success
                 final_message = get_text(user_id, 'admin_assignbots_partial_success', lang=lang,
                                          assigned_count=actually_assigned, requested_count=assigned_count,
                                          code=html.escape(code))
                 final_message += f"\nDetails: {html.escape(message)}" # Add raw details
        else: # Some other success message format? Unlikely based on db code.
             final_message = message

        await _send_or_edit_message(update, context, final_message)
        db.log_event_db("Userbots Assigned", f"Code: {code}, Attempted: {assigned_count}, Details: {message}", user_id=user_id, client_id=client_user_id)

        # Initialize/check runtime for newly assigned bots
        for phone in available_bots:
            telethon_api.get_userbot_runtime_info(phone)

    else: # Assignment failed entirely (e.g., DB error, client code disappeared)
        db.log_event_db("Bot Assign Failed", f"Code: {code}, Reason: {message}", user_id=user_id)
        fail_message = get_text(user_id, 'admin_assignbots_failed', lang=lang, code=html.escape(code)) + f"\nError: {html.escape(message)}"
        await _send_or_edit_message(update, context, fail_message)
...
async def client_folder_menu(update: Update, context: CallbackContext) -> int:
    user_id, lang = get_user_id_and_lang(update, context)
    # Clear volatile data if coming from another action via callback
    if update.callback_query:
        clear_conversation_data(context)
        lang = context.user_data.get(CTX_LANG, 'en') # Re-fetch lang
    ...
    return ConversationHandler.END # Menu shown, end state
...
async def process_folder_name(update: Update, context: CallbackContext) -> int:
    ...
    if not folder_name:
        await _send_or_edit_message(update, context, get_text(user_id, 'error_invalid_input', lang=lang))
        return STATE_WAITING_FOR_FOLDER_NAME # Stay in state if name is empty
    ...
    if isinstance(folder_id_or_status, int) and folder_id_or_status > 0:
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_create_success', lang=lang, name=html.escape(folder_name)))
        # Go back to the folder menu after successful creation
        return await client_folder_menu(update, context)
    elif folder_id_or_status is None: # Indicates duplicate name
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_create_error_exists', lang=lang, name=html.escape(folder_name)))
        return STATE_WAITING_FOR_FOLDER_NAME # Stay in state to allow renaming
    else: # Indicates DB error (-1)
        db.log_event_db("Folder Create Failed", f"Name: {folder_name}", user_id=user_id)
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_create_error_db', lang=lang))
        clear_conversation_data(context)
...
async def client_select_folder_to_edit_or_delete(update: Update, context: CallbackContext, action: str) -> int:
    ...
    folders = db.get_folders_by_user(user_id)
    if not folders:
        if query and hasattr(query, 'answer'): await query.answer(get_text(user_id, 'folder_no_folders', lang=lang), show_alert=True)
    ...
    for folder in folders_page:
        ...
        callback_action = "edit_selected" if action == 'edit' else "delete_selected"
    ...
    await _send_or_edit_message(update, context, text, reply_markup=markup)
    return STATE_WAITING_FOR_FOLDER_SELECTION # Wait for user to select a folder
...
async def client_show_folder_edit_options(update: Update, context: CallbackContext) -> int:
    ...
    # If folder_id not in context, try to extract from callback
    ...
    if not folder_id:
        ...
        return await client_folder_menu(update, context) # Go back if ID missing

    folder_name = db.get_folder_name(folder_id)
    if not folder_name:
        log.warning(f"Folder ID {folder_id} not found or user {user_id} doesn't own it (or DB error).")
        if query and hasattr(query, 'answer'): await query.answer(get_text(user_id, 'folder_not_found_error', lang=lang), show_alert=True)
        clear_conversation_data(context) # Clear potentially invalid folder ID
        return await client_folder_menu(update, context)

    context.user_data[CTX_FOLDER_NAME] = folder_name # Store name in context for later use
    ...
    if groups_in_folder:
        ...
        for i, group in enumerate(groups_in_folder):
            ...
            name = group['group_name'] or f"ID: {group['group_id']}"
    ...
    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup, disable_web_page_preview=True)
    return STATE_WAITING_FOR_FOLDER_ACTION # Wait for user to choose edit action
...
async def process_folder_links(update: Update, context: CallbackContext) -> int:
    ...
    if not raw_links:
        await _send_or_edit_message(update, context, get_text(user_id, 'join_no_links', lang=lang)) # Re-use join msg
    ...
    results = {} # url: {'status': 'added'|'ignored'|'failed', 'reason': '...' }
    added_count = 0
    failed_count = 0
    ignored_count = 0

    # Resolve links using an available active userbot for the client
    ...
    if client_bots:
        ...
        if active_client_bots:
            ...
            log.info(f"Using bot {resolver_bot_phone} for resolving folder links.")

    link_details = {} # Store resolved details {link: {'id': 123, 'name': '..'}}
    if resolver_bot_phone:
        try:
            ...
            # Proceed without resolved info, DB add will fail for non-resolvable IDs later
    ...
    for link in raw_links:
        ...
        group_name = None
        reason = None
        status_code = 'failed' # Default status
        ...
        if resolved and not resolved.get('error'):
            ...
            if group_id:
                 ...
                 if added_status is True: # Successfully added
                     status_code = 'added'; added_count += 1
                 elif added_status is None: # Duplicate, ignored by DB
                     status_code = 'ignored'; ignored_count += 1; reason = 'Duplicate in folder'
                 else: # DB Error (returned False)
                     status_code = 'failed'; reason = get_text(user_id, 'folder_add_db_error', lang=lang); failed_count += 1
            else:
                 # Resolved successfully but no ID returned (shouldn't happen with current resolver)
                 status_code = 'failed'
            ...
            # Link wasn't resolved at all (maybe no resolver bot or other issue)
            status_code = 'failed'
            reason = get_text(user_id, 'folder_resolve_error', lang=lang)
            failed_count += 1
    ...
    for link, res in results.items():
        ...
        status_text = get_text(user_id, status_key, lang=lang) # Get base status text

        # Add reason if failed or ignored
        if res['status'] != 'added' and res['reason']:
            status_text += f" ({html.escape(str(res['reason']))})"

        result_text += "\n" + get_text(user_id, 'folder_results_line', lang=lang, link=html.escape(link), status=status_text)
        displayed_count += 1

    await _send_or_edit_message(update, context, result_text, disable_web_page_preview=True)
    # Go back to the edit options menu for the same folder
...
async def client_select_groups_to_remove(update: Update, context: CallbackContext) -> int:
    ...
    if not folder_id or not folder_name:
        if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'session_expired', lang=lang), show_alert=True)
        return await client_folder_menu(update, context)
    ...
    if not groups:
        if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'folder_edit_no_groups', lang=lang), show_alert=True)
    ...
    for group in groups_page:
        ...
        # Truncate button text carefully
        max_len = 40
    ...
    action_row = []
    if selected_ids:
        confirm_text = get_text(user_id, 'folder_edit_remove_confirm_title', lang=lang) + f" ({len(selected_ids)})"
        action_row.append(InlineKeyboardButton(confirm_text, callback_data=f"{CALLBACK_FOLDER_PREFIX}edit_remove_confirm"))

    action_row.append(InlineKeyboardButton(get_text(user_id, 'button_back', lang=lang), callback_data=f"{CALLBACK_FOLDER_PREFIX}back_to_edit_options"))
    keyboard.append(action_row)

    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup)
    return STATE_FOLDER_EDIT_REMOVE_SELECT # Wait for toggle or confirm
...
async def client_toggle_group_for_removal(update: Update, context: CallbackContext) -> int:
    ...
    try:
        # Extract ID and page from callback data
    ...
    # Initialize the set in user_data if it doesn't exist
    ...
    # Regenerate the selection message for the same page
    # Modify query data to point back to the selection action for the current page
    query.data = f"{CALLBACK_FOLDER_PREFIX}edit_remove_select?page={current_page}"
...
async def client_confirm_remove_selected_groups(update: Update, context: CallbackContext) -> int:
    ...
    if not ids_to_remove:
        if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'folder_edit_remove_none_selected', lang=lang), show_alert=True)
        # Go back to selection screen if nothing was selected
        query.data = f"{CALLBACK_FOLDER_PREFIX}edit_remove_select?page=0"
        return await client_select_groups_to_remove(update, context)
    ...
    if removed_count >= 0:
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_remove_success', lang=lang, count=removed_count, name=html.escape(folder_name)))
        context.user_data.pop(CTX_TARGET_GROUP_IDS_TO_REMOVE, None) # Clear selection
        # Go back to folder edit options
        return await client_show_folder_edit_options(update, context)
    else: # DB Error (-1)
        db.log_event_db("Folder Group Remove Failed", f"Folder: {folder_name}({folder_id})", user_id=user_id)
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_remove_error', lang=lang))
        context.user_data.pop(CTX_TARGET_GROUP_IDS_TO_REMOVE, None) # Clear selection
        # Go back to folder edit options even on error
        return await client_show_folder_edit_options(update, context)
...
async def process_folder_rename(update: Update, context: CallbackContext) -> int:
    ...
    if new_name == current_name:
        # If name hasn't changed, just go back to the options menu
        return await client_show_folder_edit_options(update, context)
    ...
    if success:
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_rename_success', lang=lang, new_name=html.escape(new_name)))
        context.user_data[CTX_FOLDER_NAME] = new_name # Update context with new name
        return await client_show_folder_edit_options(update, context) # Back to edit options
    else:
        if reason == "name_exists":
            await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_rename_error_exists', lang=lang, new_name=html.escape(new_name)))
            return STATE_FOLDER_RENAME_PROMPT # Stay in state to try again
        else: # DB error or not found/unauthorized
            db.log_event_db("Folder Rename Failed", f"ID: {folder_id}, To: {new_name}, Reason: {reason}", user_id=user_id)
            await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_rename_error_db', lang=lang))
            # Go back to edit options even on error
            return await client_show_folder_edit_options(update, context)
...
async def client_confirm_folder_delete(update: Update, context: CallbackContext) -> int:
    ...
    folder_name = db.get_folder_name(folder_id)
    if not folder_name:
        if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'folder_not_found_error', lang=lang), show_alert=True)
        return await client_folder_menu(update, context)
    ...
    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup)
    # Keep same state, waiting for yes/no callback
...
async def client_delete_folder_confirmed(update: Update, context: CallbackContext) -> int:
    ...
    if db.delete_folder(folder_id, user_id):
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_delete_success', lang=lang, name=html.escape(folder_name or '')))
    else:
        log.warning(f"Failed delete folder ID {folder_id} by user {user_id}")
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_delete_error', lang=lang))

    # Go back to the main folder menu regardless of success/fail
    return await client_folder_menu(update, context)
...
async def client_select_bot_generic(update: Update, context: CallbackContext, action_prefix: str, next_state: str, title_key: str) -> int | None:
    ...
    if not user_bots:
        if query and hasattr(query, 'answer'): await query.answer(get_text(user_id, 'client_menu_no_userbots', lang=lang), show_alert=True)
        return await client_menu(update, context) # Back to main menu if no bots
    ...
    # Add 'All Bots'/'Active Bots' options only for Join action
    if action_prefix == CALLBACK_JOIN_PREFIX:
        ...
        if active_bots_count > 0: # Show 'Active' only if there are any
             keyboard.append([InlineKeyboardButton(get_text(user_id, 'join_select_userbot_active', lang=lang, count=active_bots_count), callback_data=f"{action_prefix}select_active")])
    ...
    await _send_or_edit_message(update, context, text, reply_markup=markup)
    return STATE_WAITING_FOR_USERBOT_SELECTION # Wait for selection

async def handle_userbot_selection(update: Update, context: CallbackContext, action_prefix: str, next_state: str) -> int | None:
    query = update.callback_query
    user_id, lang = get_user_id_and_lang(update, context)
    data = query.data
    selected_option = data.split(f"{action_prefix}select_")[1]

    selected_bots = []
    if selected_option == 'all':
        selected_bots = db.get_client_bots(user_id)
        if not selected_bots:
            if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'client_menu_no_userbots', lang=lang), show_alert=True)
            return await client_menu(update, context)
        ...
        if not selected_bots:
            if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'join_no_active_bots', lang=lang), show_alert=True)
            # Go back to selection? Or main menu? Let's go back to bot selection
            return await client_select_bot_generic(update, context, action_prefix, next_state, 'join_select_userbot' if action_prefix == CALLBACK_JOIN_PREFIX else 'task_select_userbot')
    else: # Specific phone selected
        phone = selected_option
        bot_info = db.find_userbot(phone)
        # Verify the selected bot actually belongs to the user
        if not bot_info or phone not in db.get_client_bots(user_id):
            ...
            if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'error_invalid_input', lang=lang), show_alert=True)
    ...
    # Proceed based on action prefix
    if action_prefix == CALLBACK_JOIN_PREFIX:
        await _send_or_edit_message(update, context, get_text(user_id, 'join_enter_group_links', lang=lang))
        return STATE_WAITING_FOR_GROUP_LINKS
    elif action_prefix == CALLBACK_TASK_PREFIX:
        context.user_data[CTX_TASK_PHONE] = selected_bots[0] # Store single bot for task setup
...
async def process_join_group_links(update: Update, context: CallbackContext) -> int:
    ...
    if not raw_links:
        await _send_or_edit_message(update, context, get_text(user_id, 'join_no_links', lang=lang))
        return STATE_WAITING_FOR_GROUP_LINKS # Stay in state
    ...
    for i, result_item in enumerate(results_list):
        ...
        if error_info and error_info.get("error"):
            ...
            # Don't process individual results if there was a fatal batch error
            continue

        if not results_dict:
            all_results_text += f"\n  -> ({get_text(user_id, 'error_no_results', lang=lang)})"
            continue

        processed_links_count = 0
        for link, (status, detail) in results_dict.items():
             # Get base status text (e.g., "Joined", "Failed", "Already Member")
             status_key = f"join_results_{status}"
             status_text = get_text(user_id, status_key, lang=lang) # Default to key if not found

             # Append reason/details if status is not success/already_member
             if status not in ['success', 'already_member'] and isinstance(detail, dict):
                  ...
                  reason_text = ""
                  if reason:
                      reason_key = f"join_results_reason_{reason}"
                      reason_base_text = get_text(user_id, reason_key, lang=lang)
                      if reason_base_text != reason_key: # Found translation for reason code
                          reason_text = reason_base_text.format(error=html.escape(str(error or '')), seconds=seconds or '')
                      else: # No translation, use raw reason code
                          reason_text = html.escape(str(reason))
                          if error: reason_text += f" ({html.escape(str(error))})"
                  elif error: # If no reason code but error message exists
                       reason_text = html.escape(str(error))

                  if reason_text:
                       # Format the status text for failures
                       status_text = get_text(user_id, 'join_results_failed', lang=lang, reason=reason_text)
                  elif status == 'flood_wait' and seconds:
                       # Specific formatting for flood wait
                       status_text = get_text(user_id, 'join_results_flood_wait', lang=lang, seconds=seconds)
             ...
             if len(all_results_text) > 3800:
                 ...
                 break # Stop processing results for this bot
    ...
    if len(all_results_text) > 4096:
        ...
        for i, part in enumerate(parts):
            part_markup = markup if i == len(parts) - 1 else None # Add button only to last part
            try:
                ...
                if i < len(parts) - 1: await asyncio.sleep(0.5) # Small delay between parts
                 ...
                 # Try sending remaining parts? Or just stop? Let's stop here.
                 break
    else:
        await _send_or_edit_message(update, context, all_results_text, reply_markup=markup, disable_web_page_preview=True)
...
async def client_show_stats(update: Update, context: CallbackContext) -> int:
    ...
    if not stats:
        ...
        # Show only total messages for now, as others are unreliable/hard to track
    ...
    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup, parse_mode=ParseMode.HTML)
    return ConversationHandler.END # Just showing info
...
async def task_show_settings_menu(update: Update, context: CallbackContext) -> int:
    ...
    # Load existing settings or initialize if not found
    task_settings_db = db.get_userbot_task_settings(user_id, phone)
    if task_settings_db:
        # If settings exist in DB but not in context (e.g., re-entering menu), load them
        if CTX_TASK_SETTINGS not in context.user_data:
            context.user_data[CTX_TASK_SETTINGS] = dict(task_settings_db)
    else:
        # If no settings in DB, initialize empty dict in context
        if CTX_TASK_SETTINGS not in context.user_data:
            context.user_data[CTX_TASK_SETTINGS] = {}

    # Use settings from context (might have unsaved changes)
    ...
    status_icon = get_text(user_id, status_icon_key, lang=lang) # Uses 'en' fallback if key missing
    ...
    start_time_str = format_dt(start_time_ts, fmt='%H:%M') # Use helper
    ...
    if interval_min:
         ...
         else: interval_disp = f"{interval_min // 60} h {interval_min % 60} min" # Show hours and minutes
    ...
    if current_settings.get('send_to_all_groups'):
        ...
        folder_id = current_settings['folder_id']
        folder_name = db.get_folder_name(folder_id)
        if folder_name:
            target_str = get_text(user_id, 'task_value_folder', lang=lang, name=html.escape(folder_name))
        else:
            # Folder might have been deleted
            target_str = get_text(user_id, 'task_value_folder', lang=lang, name=f"ID: {folder_id}") + " (Deleted?)"

    last_run_str = format_dt(current_settings.get('last_run')) # Use helper
    last_error_raw = current_settings.get('last_error')
    last_error = html.escape(last_error_raw[:100]) + ('...' if last_error_raw and len(last_error_raw) > 100 else '') if last_error_raw else get_text(user_id, 'task_value_not_set', lang=lang)
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(get_text(user_id, 'task_button_deactivate' if status == 'active' else 'task_button_activate', lang=lang), callback_data=f"{CALLBACK_TASK_PREFIX}toggle_status"),
    ...
    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup, disable_web_page_preview=True)
    return STATE_TASK_SETUP # Stay in main task setup state
...
async def process_task_link(update: Update, context: CallbackContext, link_type: str) -> int:
    ...
    # Handle skip for fallback link
    if link_type == 'fallback' and link_text.lower() == 'skip':
        task_settings['fallback_message_link'] = None
        await _send_or_edit_message(update, context, get_text(user_id, 'task_set_skipped_fallback', lang=lang))
        return await task_show_settings_menu(update, context) # Back to menu
    ...
    if link_parsed_type != "message_link":
        await _send_or_edit_message(update, context, get_text(user_id, 'task_error_invalid_link', lang=lang))
        return next_state # Stay in state

    # Verify link access using Telethon
    link_verified = False
    await _send_or_edit_message(update, context, get_text(user_id, 'task_verifying_link', lang=lang)) # Inform user
    try:
        ...
        if not accessible:
            ...
            await _send_or_edit_message(update, context, get_text(user_id, 'task_error_link_unreachable', lang=lang, bot_phone=html.escape(phone)))
            return next_state # Stay in state
        else:
         ...
         await _send_or_edit_message(update, context, get_text(user_id, 'error_telegram_api', lang=lang, error=html.escape(str(e))))
         return next_state # Stay in state on error

    # --- Store the link if verified ---
    if link_verified:
        if link_type == 'primary':
            task_settings['message_link'] = link_text
            await _send_or_edit_message(update, context, get_text(user_id, 'task_set_success_msg', lang=lang))
            # Prompt for fallback link? Let's skip for simplicity now and go back to menu.
            # await _send_or_edit_message(update, context, get_text(user_id, 'task_prompt_fallback_link', lang=lang))
            # return STATE_WAITING_FOR_FALLBACK_MESSAGE_LINK
            return await task_show_settings_menu(update, context) # Back to menu
        else: # Fallback
            task_settings['fallback_message_link'] = link_text
            await _send_or_edit_message(update, context, get_text(user_id, 'task_set_success_fallback', lang=lang))
            return await task_show_settings_menu(update, context) # Back to menu
    else:
        # This path should ideally not be reached due to returns above, but is a safeguard
        log.error(f"Link verification did not complete successfully for {link_text}. Not storing link.")
...
async def process_task_start_time(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if not (0 <= hour <= 23 and 0 <= minute <= 59): raise ValueError("Invalid hour/minute")
    except (ValueError, TypeError):
    ...
    # Convert local HH:MM to next occurrence's UTC timestamp
    try:
        now_local = datetime.now(LITHUANIA_TZ)
        input_time_obj = datetime.strptime(f"{hour:02}:{minute:02}", "%H:%M").time()
        ...
        # If the target time is already past for today, set it for tomorrow
        if target_local_dt <= now_local:
            target_local_dt += timedelta(days=1)

        # Convert the final local target time to UTC and get timestamp
        ...
        log.info(f"User {user_id} set task start time: {time_str} LT -> {start_timestamp} UTC ({target_utc.strftime('%Y-%m-%d %H:%M')})")
        await _send_or_edit_message(update, context, get_text(user_id, 'task_set_success_time', lang=lang, time=time_str))
        return await task_show_settings_menu(update, context) # Back to menu
        ...
        return STATE_WAITING_FOR_START_TIME # Stay in state on conversion error

async def task_select_interval(update: Update, context: CallbackContext) -> int:
    query = update.callback_query
    user_id, lang = get_user_id_and_lang(update, context)
    # Define intervals in minutes
    intervals = [5, 10, 15, 30, 60, 120, 180, 240, 360, 720, 1440] # Up to 1 day
    keyboard = []
    row = []
    ...
    await _send_or_edit_message(update, context, get_text(user_id, 'task_select_interval_title', lang=lang), reply_markup=markup)
    # Stay in main task setup state, wait for interval callback
...
async def process_interval_callback(update: Update, context: CallbackContext) -> int:
    ...
    if hasattr(query, 'answer'): await query.answer() # Answer silently
    # Update the main settings menu immediately
    return await task_show_settings_menu(update, context)
...
async def task_select_target_type(update: Update, context: CallbackContext) -> int:
    ...
    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, get_text(user_id, 'task_select_target_title', lang=lang), reply_markup=markup)
    # Stay in main task setup state, waiting for callback
    return STATE_TASK_SETUP
...
async def task_select_folder_for_target(update: Update, context: CallbackContext) -> int:
    ...
    folders = db.get_folders_by_user(user_id)
    if not folders:
        if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'task_error_no_folders', lang=lang), show_alert=True)
        return await task_select_target_type(update, context) # Go back to target type selection
    ...
    markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup)
    # Need a state to wait for folder selection callback
    return STATE_SELECT_TARGET_GROUPS
...
async def task_set_target(update: Update, context: CallbackContext, target_type: str) -> int:
    ...
    if target_type == 'all':
        ...
        task_settings['folder_id'] = None # Explicitly set folder ID to None
        ...
        try: _, params = query.data.split('?', 1); folder_id = int(params.split('=')[1])
            ...
            if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True)
            return STATE_TASK_SETUP # Go back to task setup menu
        ...
        return STATE_TASK_SETUP # Back to menu on error

    if hasattr(query, 'answer'): await query.answer() # Answer silently
    # Update the main settings menu
    return await task_show_settings_menu(update, context)
...
async def task_toggle_status(update: Update, context: CallbackContext) -> int:
    ...
    # Validate required fields only if activating
    if new_status == 'active':
        ...
        if missing_fields:
            ...
            if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'task_save_validation_fail', lang=lang, missing=missing_str), show_alert=True)
            return STATE_TASK_SETUP # Stay in menu, don't change status

    task_settings['status'] = new_status
    log.info(f"User {user_id} toggled task status to {new_status} (unsaved)")
    if hasattr(query, 'answer'): await query.answer() # Answer silently
    # Update the menu to reflect the changed status
    return await task_show_settings_menu(update, context)
...
async def task_save_settings(update: Update, context: CallbackContext) -> int:
    ...
    # Final validation before saving (especially if activating)
    if settings_to_save.get('status') == 'active':
        ...
        if missing_fields:
            missing_str = ", ".join(missing_fields)
            if hasattr(query, 'answer'): await query.answer(get_text(user_id, 'task_save_validation_fail', lang=lang, missing=missing_str), show_alert=True)
            return STATE_TASK_SETUP # Stay in menu if validation fails

    # Clear last error on manual save
    ...
    if db.save_userbot_task_settings(user_id, phone, settings_to_save):
        ...
        clear_conversation_data(context) # Clear task setup data
        return await client_menu(update, context) # Back to main menu
    else:
        db.log_event_db("Task Save Failed", f"User: {user_id}, Bot: {phone}", user_id=user_id, userbot_phone=phone)
        await _send_or_edit_message(update, context, get_text(user_id, 'task_save_error', lang=lang))
        return STATE_TASK_SETUP # Stay in menu on save error
...
async def admin_list_userbots(update: Update, context: CallbackContext) -> int:
    ...
    all_bots = db.get_all_userbots() # Fetches all bots with client info joined
    if not all_bots:
    ...
    for bot in bots_page:
        ...
        # Get status icon from translations or fallback map
        ...
        text += get_text(user_id, 'admin_userbot_list_line', lang=lang,
    ...
    await _send_or_edit_message(update, context, text, reply_markup=markup, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    return ConversationHandler.END # Just showing info
...
async def admin_select_userbot_to_remove(update: Update, context: CallbackContext) -> int:
    ...
    for bot in bots_page:
        ...
        # Include page in confirmation callback to potentially return to the right page later? Not strictly necessary.
    ...
    await _send_or_edit_message(update, context, text, reply_markup=markup)
    # Need a state to wait for the confirmation callback
    return STATE_ADMIN_CONFIRM_USERBOT_RESET # Re-using this state name, though it's just for remove confirm
...
async def admin_confirm_remove_userbot(update: Update, context: CallbackContext) -> int:
     ...
     keyboard = [[
         ...
         InlineKeyboardButton(get_text(user_id, 'button_no', lang=lang), callback_data=f"{CALLBACK_ADMIN_PREFIX}back_to_menu") # Go back to admin menu if 'No'
     ]]
     markup = InlineKeyboardMarkup(keyboard)
     await _send_or_edit_message(update, context, text, reply_markup=markup)
     # Stay in the same state, waiting for yes/no callback
...
async def admin_remove_userbot_confirmed(update: Update, context: CallbackContext) -> int:
    ...
    # 1. Stop the Telethon runtime for the userbot
    ...
    # 2. Remove userbot from database
    if db.remove_userbot(phone_to_remove):
        # 3. Delete session files AFTER successful DB removal
        log.info(f"Attempting to remove session files for {phone_to_remove}...")
        telethon_api.delete_session_files_for_phone(phone_to_remove) # Call the renamed public function

        db.log_event_db("Userbot Removed", f"Phone: {phone_to_remove}", user_id=user_id, userbot_phone=phone_to_remove)
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_remove_success', lang=lang, display_name=display_name))
    else:
        # DB removal failed (e.g., bot didn't exist)
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_remove_error', lang=lang))
...
async def admin_view_subscriptions(update: Update, context: CallbackContext) -> int:
    ...
    subs = db.get_all_subscriptions() # Gets activated clients with bot count
    if not subs:
...
async def admin_view_system_logs(update: Update, context: CallbackContext) -> int:
    query = update.callback_query
    user_id, lang = get_user_id_and_lang(update, context)
    limit = 25 # Show last 25 logs

    # Pagination not implemented for logs currently, ignore page param if present
    # try: _, params = query.data.split('?', 1); current_page = int(params.split('=')[1])
    ...
    for log_entry in logs:
        ...
        user_str = get_text(user_id, 'admin_logs_user_none', lang=lang) # Default 'System'
        if log_user_id:
            user_str = get_text(user_id, 'admin_logs_user_admin', lang=lang) + f" ({log_user_id})" if is_admin(log_user_id) else f"{log_user_id}"
...
async def handle_client_callback(update: Update, context: CallbackContext) -> str | int | None:
    ...
    # Check if client is valid and subscription active
    if not client_info or client_info['subscription_end'] < int(datetime.now(UTC_TZ).timestamp()):
        ...
        if query and hasattr(query,'answer'): await query.answer(get_text(user_id, 'subscription_expired', lang=lang), show_alert=True)
        clear_conversation_data(context) # Clear data for expired user
        return ConversationHandler.END
    ...
    if action == "select_bot_task":
        ...
        clear_conversation_data(context) # Clear data when returning to main menu
        return await client_menu(update, context)
    else:
        log.warning(f"Unhandled CLIENT CB: Action='{action}', Data='{data}'")
        if query and hasattr(query,'answer'): await query.answer("Action not recognized.", show_alert=True)
    return None # Return None if action doesn't change state or end conversation
...
async def handle_admin_callback(update: Update, context: CallbackContext) -> str | int | None:
    ...
    if not is_admin(user_id):
        if query and hasattr(query,'answer'): await query.answer(get_text(user_id, 'unauthorized', lang=lang), show_alert=True)
        return ConversationHandler.END

    data = query.data
    # Handle actions with dynamic parts (like phone numbers)
    if data.startswith(f"{CALLBACK_ADMIN_PREFIX}remove_bot_confirm_"):
        action = "remove_bot_confirm"
    elif data.startswith(f"{CALLBACK_ADMIN_PREFIX}remove_bot_confirmed_"):
        action = "remove_bot_confirmed"
    else:
        action = data.split(CALLBACK_ADMIN_PREFIX)[1].split('?')[0] # Get action part

    log.debug(f"Admin Callback Route: Action='{action}', Data='{data}'")

    if action == "add_bot_prompt":
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_prompt_phone', lang=lang))
        return STATE_WAITING_FOR_PHONE
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_invite_prompt_details', lang=lang))
        return STATE_WAITING_FOR_SUB_DETAILS
    elif action == "view_subs":
        return await admin_view_subscriptions(update, context)
    elif action == "extend_sub_prompt":
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_extend_prompt_code', lang=lang))
        return STATE_WAITING_FOR_EXTEND_CODE
    elif action == "assign_bots_prompt":
        await _send_or_edit_message(update, context, get_text(user_id, 'admin_assignbots_prompt_code', lang=lang))
        return STATE_WAITING_FOR_ADD_USERBOTS_CODE
...
async def handle_folder_callback(update: Update, context: CallbackContext) -> str | int | None:
    ...
    if action == "create_prompt":
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_create_prompt', lang=lang))
        ...
        clear_conversation_data(context) # Clear folder context when going back
        return await client_folder_menu(update, context)
    elif action == "edit_add_prompt":
        folder_name = context.user_data.get(CTX_FOLDER_NAME, "this folder")
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_add_prompt', lang=lang, name=html.escape(folder_name)))
        return STATE_WAITING_FOR_GROUP_LINKS # Use same state as join for adding links
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'folder_edit_rename_prompt', lang=lang, current_name=html.escape(current_name)))
        return STATE_FOLDER_RENAME_PROMPT
    elif action == "back_to_edit_options":
        # Clear selection if going back from remove screen
        context.user_data.pop(CTX_TARGET_GROUP_IDS_TO_REMOVE, None)
        return await client_show_folder_edit_options(update, context)
    else:
        log.warning(f"Unhandled FOLDER CB: Action='{action}', Data='{data}'")
        if hasattr(query, 'answer'): await query.answer("Action not recognized.", show_alert=True)
    return None
...
async def handle_task_callback(update: Update, context: CallbackContext) -> str | int | None:
    ...
    # Handle userbot selection for task setup
    if action.startswith("select_"):
        ...
        clear_conversation_data(context) # Clear task data when going back
        return await client_select_bot_generic(update, context, CALLBACK_TASK_PREFIX, STATE_TASK_SETUP, 'task_select_userbot')
    elif action == "back_to_task_menu":
        return await task_show_settings_menu(update, context)
    elif action == "set_primary_link":
        return await task_prompt_set_link(update, context, 'primary')
    # Fallback link setting removed for simplicity for now
...
async def handle_generic_callback(update: Update, context: CallbackContext) -> str | int | None:
    ...
    if action == "cancel":
        clear_conversation_data(context) # Ensure data is cleared
        await _send_or_edit_message(update, context, get_text(user_id, 'cancelled', lang=lang))
        # Try to return to appropriate menu
        ...
        clear_conversation_data(context) # Clear data on 'No' as well
        await _send_or_edit_message(update, context, get_text(user_id, 'cancelled', lang=lang))
        ...
        if query and hasattr(query,'answer') and not context.bot_data.get(f'answered_{query.id}', False):
             try:
                 await query.answer()
                 context.bot_data[f'answered_{query.id}'] = True
             except Exception: pass
        return None # Explicitly return None for noop to indicate no state change
    else:
...
async def main_callback_handler(update: Update, context: CallbackContext) -> str | int | None:
    ...
    next_state = None # Default: stay in current state or no change
    ...
    try:
        if data.startswith(CALLBACK_CLIENT_PREFIX):
            ...
            # Avoid answering if already answered by handle_generic_callback (e.g., noop)
            ...
            next_state = ConversationHandler.END # End conversation on unknown button
        ...
        await _send_or_edit_message(update, context, get_text(user_id, 'error_generic', lang=lang))
        clear_conversation_data(context) # Clear data on unhandled error
...
# Defines the states and transitions for multi-step interactions.
main_conversation = ConversationHandler(
    entry_points=[
        ...
        # Route all callback queries directly to the main router if no state is active
        CallbackQueryHandler(main_callback_handler)
    ],
    states={
        # Initial client activation state
        STATE_WAITING_FOR_CODE: [MessageHandler(Filters.text & ~Filters.command & Filters.chat_type.private, process_invitation_code)],

        # Admin States (Userbot Add, Invite Gen, Sub Extend, Bot Assign)
        ...
        STATE_ADMIN_CONFIRM_USERBOT_RESET: [CallbackQueryHandler(main_callback_handler)], # For remove confirmation
        ...
        STATE_WAITING_FOR_FOLDER_ACTION: [CallbackQueryHandler(main_callback_handler)], # Routes folder edit actions
        STATE_FOLDER_RENAME_PROMPT: [MessageHandler(Filters.text & ~Filters.command & Filters.chat_type.private, process_folder_rename)],
        STATE_FOLDER_EDIT_REMOVE_SELECT: [CallbackQueryHandler(main_callback_handler)], # For toggling/confirming removal
        STATE_WAITING_FOR_GROUP_LINKS: [MessageHandler(Filters.text & ~Filters.command & Filters.chat_type.private, process_folder_links)], # Re-used for folder add/join
        STATE_WAITING_FOR_FOLDER_SELECTION: [CallbackQueryHandler(main_callback_handler)], # For selecting folder to edit/delete/confirm delete
        ...
        STATE_SELECT_TARGET_GROUPS: [CallbackQueryHandler(main_callback_handler)], # For selecting target folder

        # Generic State for selecting userbot (used by Join and Task Setup)
        STATE_WAITING_FOR_USERBOT_SELECTION: [CallbackQueryHandler(main_callback_handler)],

        # Note: STATE_WAITING_FOR_LANGUAGE is handled directly by callback, no state needed.
    },
    fallbacks=[
        ...
        # Fallback Callback Handler (catches callbacks in unexpected states)
        CallbackQueryHandler(main_callback_handler),
        # Fallback Message Handler (catches any non-command text in unexpected states)
        MessageHandler(Filters.text & ~Filters.command & Filters.chat_type.private, conversation_fallback),
    ],
    allow_reentry=True, # Allows re-entering states via commands like /start
    # Optional: Add persistence layer here if needed
