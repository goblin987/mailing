...
def clear_conversation_data(context: CallbackContext):
    ...
    keys_to_clear = [ CTX_PHONE, CTX_API_ID, CTX_API_HASH, CTX_AUTH_DATA, CTX_INVITE_DETAILS, CTX_EXTEND_CODE, CTX_ADD_BOTS_CODE, CTX_FOLDER_ID, CTX_FOLDER_NAME, CTX_FOLDER_ACTION, CTX_SELECTED_BOTS, CTX_TARGET_GROUP_IDS_TO_REMOVE, CTX_TASK_PHONE, CTX_TASK_SETTINGS, CTX_PAGE, CTX_MESSAGE_ID ]
...
async def _send_or_edit_message(update: Update, context: CallbackContext, text: str, **kwargs):
    ...
    sent_message = None # To store the result
    ...
    try:
        if query:
            ...
            try:
                ...
                # Check existence of 'answered' attribute for safety
                if hasattr(query, 'answered') and not query.answered:
                    await query.answer()
                answered_callback = True
            except (AttributeError, BadRequest, TelegramError) as cb_e: # Catch AttributeError too
        ...
        if "message is not modified" in str(e).lower():
            ...
            if query and hasattr(query, 'answered') and not answered_callback and not query.answered:
...
def format_dt(timestamp: int | None, tz=LITHUANIA_TZ, fmt='%Y-%m-%d %H:%M') -> str:
    ...
    except (ValueError, TypeError, OSError): log.warning(f"Could not format invalid timestamp: {timestamp}"); return "Invalid Date"
...
def build_client_menu(user_id, context: CallbackContext):
    ...
    if userbot_phones:
        for i, phone in enumerate(userbot_phones, 1):
            ...
            status_icon = "🟢" if bot_db_info and bot_db_info['status'] == 'active' else "🟡" if bot_db_info and bot_db_info['status'] not in ['active', 'inactive', 'error'] else "🔴" if bot_db_info and bot_db_info['status'] == 'error' else "⚪️"
            menu_text += f"{i}. {status_icon} {display_name} (<i>Status: {html.escape(status)}</i>)\n"
            if last_error: escaped_error = html.escape(last_error); menu_text += f"  └─ <pre>Error: {escaped_error[:100]}{'...' if len(escaped_error)>100 else ''}</pre>\n"
    else: menu_text += get_text(user_id, 'client_menu_no_userbots', lang=lang) + "\n"
    keyboard = [ [InlineKeyboardButton(get_text(user_id, 'client_menu_button_setup_tasks', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}select_bot_task")], [InlineKeyboardButton(get_text(user_id, 'client_menu_button_manage_folders', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}manage_folders")], [ InlineKeyboardButton(get_text(user_id, 'client_menu_button_join_groups', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}select_bot_join"), ], [ InlineKeyboardButton(get_text(user_id, 'client_menu_button_language', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}language") ], ]
...
def build_pagination_buttons(base_callback_data: str, current_page: int, total_items: int, items_per_page: int) -> list:
    buttons = []; total_pages = math.ceil(total_items / items_per_page)
    if total_pages <= 1: return []
    row = []
    if current_page > 0: prev_text = get_text(0, 'pagination_prev', lang='en'); row.append(InlineKeyboardButton(prev_text, callback_data=f"{base_callback_data}?page={current_page - 1}"))
    if total_pages > 1: page_text = get_text(0,'pagination_page',lang='en').format(current=current_page + 1, total=total_pages); row.append(InlineKeyboardButton(page_text, callback_data=f"{CALLBACK_GENERIC_PREFIX}noop"))
    if current_page < total_pages - 1: next_text = get_text(0, 'pagination_next', lang='en'); row.append(InlineKeyboardButton(next_text, callback_data=f"{base_callback_data}?page={current_page + 1}"))
...
async def start_command(update: Update, context: CallbackContext) -> str | int:
    user_id, lang = get_user_id_and_lang(update, context); clear_conversation_data(context); log.info(f"Start cmd: UserID={user_id}, User={update.effective_user.username}")
    if is_admin(user_id): log.info(f"Admin user {user_id} used /start, showing admin menu."); return await admin_command(update, context)
    client_info = db.find_client_by_user_id(user_id)
    if client_info:
        now_ts = int(datetime.now(UTC_TZ).timestamp())
        if client_info['subscription_end'] < now_ts: await _send_or_edit_message(update, context, get_text(user_id, 'subscription_expired', lang=lang)); return ConversationHandler.END
        else: return await client_menu(update, context)
    else: await _send_or_edit_message(update, context, get_text(user_id, 'welcome', lang=lang)); return STATE_WAITING_FOR_CODE

async def process_invitation_code(update: Update, context: CallbackContext) -> str | int:
    user_id, lang = get_user_id_and_lang(update, context); code = update.message.text.strip().lower(); log.info(f"UserID={user_id} submitted code: {code}")
    ...
    success, status_key = db.activate_client(code, user_id); text_to_send = get_text(user_id, status_key, lang=lang)
    if success: log.info(f"Activated client {user_id} code {code}"); db.log_event_db("Client Activated", f"Code: {code}", user_id=user_id); context.user_data[CTX_LANG] = db.get_user_language(user_id); lang = context.user_data[CTX_LANG]; await _send_or_edit_message(update, context, text_to_send); return await client_menu(update, context)
    else: log.warning(f"Failed activation user {user_id} code {code}: {status_key}"); await _send_or_edit_message(update, context, text_to_send); clear_conversation_data(context); return ConversationHandler.END

async def admin_command(update: Update, context: CallbackContext) -> str | int:
    user_id, lang = get_user_id_and_lang(update, context); clear_conversation_data(context); log.info(f"Admin cmd: UserID={user_id}, User={update.effective_user.username}")
    if not is_admin(user_id): await _send_or_edit_message(update, context, get_text(user_id, 'unauthorized', lang=lang)); return ConversationHandler.END
    title, markup, parse_mode = build_admin_menu(user_id, context); await _send_or_edit_message(update, context, title, reply_markup=markup, parse_mode=parse_mode); return ConversationHandler.END

async def cancel_command(update: Update, context: CallbackContext) -> int:
    user_id, lang = get_user_id_and_lang(update, context); log.info(f"Cancel cmd: UserID={user_id}"); current_state = context.user_data.get(ConversationHandler.CURRENT_STATE if hasattr(ConversationHandler, 'CURRENT_STATE') else '_user_data') if context.user_data else 'None'; log.debug(f"Cancel called from state: {current_state}")
    await _send_or_edit_message(update, context, get_text(user_id, 'cancelled', lang=lang)); clear_conversation_data(context); return ConversationHandler.END

async def conversation_fallback(update: Update, context: CallbackContext) -> int:
     user_id, lang = get_user_id_and_lang(update, context); state = context.user_data.get(ConversationHandler.CURRENT_STATE if hasattr(ConversationHandler, 'CURRENT_STATE') else '_user_data') if context.user_data else 'None'
     msg_text = update.effective_message.text if update.effective_message else 'Non-text update'; log.warning(f"Conv fallback: UserID={user_id}. State={state}. Msg='{msg_text[:50]}...'")
     if update.message and update.message.text and update.message.text.startswith('/'):
         if update.message.text == '/cancel': return await cancel_command(update, context)
         if update.message.text == '/start': await _send_or_edit_message(update, context, get_text(user_id, 'state_cleared', lang=lang)); clear_conversation_data(context); return await start_command(update, context)
         if update.message.text == '/admin' and is_admin(user_id): await _send_or_edit_message(update, context, get_text(user_id, 'state_cleared', lang=lang)); clear_conversation_data(context); return await admin_command(update, context)
     await _send_or_edit_message(update, context, get_text(user_id, 'conversation_fallback', lang=lang)); clear_conversation_data(context); return ConversationHandler.END
...
async def client_menu(update: Update, context: CallbackContext) -> int:
    ...
    message, markup, parse_mode = build_client_menu(user_id, context); await _send_or_edit_message(update, context, message, reply_markup=markup, parse_mode=parse_mode); return ConversationHandler.END
...
async def set_language_handler(update: Update, context: CallbackContext) -> int:
    ...
    async def answer_callback_task(text, show_alert=False):
        try:
             if hasattr(query, 'answered') and not query.answered: await query.answer(text, show_alert=show_alert)
    ...
    if db.set_user_language(user_id, lang_code):
         ...
         await _send_or_edit_message(update, context, get_text(user_id, 'language_set', lang=lang, lang_name=language_names[lang_code]), reply_markup=InlineKeyboardMarkup([[ InlineKeyboardButton(get_text(user_id, 'button_main_menu', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}back_to_menu")]]))
...
async def process_admin_api_id(update: Update, context: CallbackContext) -> str | int:
    ...
    try: api_id = int(api_id_str); assert api_id > 0; context.user_data[CTX_API_ID] = api_id; log.info(f"Admin {user_id} API ID OK for {context.user_data.get(CTX_PHONE)}")
    except (ValueError, TypeError, AssertionError): await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_invalid_api_id', lang=lang)); return STATE_WAITING_FOR_API_ID
    await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_prompt_api_hash', lang=lang)); return STATE_WAITING_FOR_API_HASH
...
async def process_admin_api_hash(update: Update, context: CallbackContext) -> str | int:
    ...
    await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_connecting', lang=lang, phone=phone))
    try:
        ...
        if auth_status == 'code_needed': context.user_data[CTX_AUTH_DATA] = auth_data; await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_prompt_code', lang=lang, phone=phone)); return STATE_WAITING_FOR_CODE_USERBOT
            ...
            error_msg = auth_data.get('error_message', 'Unknown error'); log.error(f"Auth start error for {phone}: {error_msg}"); locals_for_format = {'phone': phone, 'error': error_msg}; key = 'admin_userbot_auth_error_unknown'
    ...
    except Exception as e: log.error(f"Exception during start_authentication_flow for {phone}: {e}", exc_info=True); await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_error_unknown', lang=lang, phone=phone, error=str(e))); clear_conversation_data(context); return ConversationHandler.END
...
async def process_admin_userbot_code(update: Update, context: CallbackContext) -> str | int:
    ...
    if not code.isdigit(): await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_error_code_invalid', lang=lang, phone=phone, error="Format incorrect")); return STATE_WAITING_FOR_CODE_USERBOT
    await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_signing_in', lang=lang, phone=phone))
    try:
        ...
        if comp_status == 'success':
            ...
            safe_phone_part = re.sub(r'[^\d]', '', phone_num); session_file_rel = f"{safe_phone_part or f'unknown_{random.randint(1000,9999)}'}.session"
            ...
            await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_add_success', lang=lang, display_name=display_name))
            asyncio.create_task(telethon_api.get_userbot_runtime_info(phone_num)); clear_conversation_data(context); return ConversationHandler.END
            ...
            error_msg = comp_data.get('error_message', 'Unknown error.'); log.error(f"Auth code completion error for {phone}: {error_msg}"); locals_for_format = {'phone': phone, 'error': error_msg}; key = 'admin_userbot_auth_error_unknown'
            ...
            await _send_or_edit_message(update, context, get_text(user_id, key, lang=lang, **locals_for_format)); clear_conversation_data(context); return ConversationHandler.END
    except Exception as e: log.error(f"Exception during complete_authentication_flow (code) for {phone}: {e}", exc_info=True); context.user_data.pop(CTX_AUTH_DATA, None); await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_auth_error_unknown', lang=lang, phone=phone, error=str(e))); clear_conversation_data(context); return ConversationHandler.END
...
async def process_admin_userbot_password(update: Update, context: CallbackContext) -> str | int:
    ...
    try:
        ...
        if comp_status == 'success':
            ...
            safe_phone_part = re.sub(r'[^\d]', '', phone_num); session_file_rel = f"{safe_phone_part or f'unknown_{random.randint(1000,9999)}'}.session"
            db_ok = db.add_userbot(phone_num, session_file_rel, api_id, api_hash, 'active', username, None, None)
            if db_ok: log.info(f"Userbot {phone_num} added/updated in DB after password auth.")
            else: log.error(f"Auth OK for {phone_num}, but DB add/update failed.")
            await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_add_success', lang=lang, display_name=display_name))
            asyncio.create_task(telethon_api.get_userbot_runtime_info(phone_num)); clear_conversation_data(context); return ConversationHandler.END
        else:
            error_msg = comp_data.get('error_message', 'Unknown error.'); log.error(f"Auth password completion error for {phone}: {error_msg}"); locals_for_format = {'phone': phone, 'error': error_msg}; key = 'admin_userbot_auth_error_unknown'
...
async def process_admin_invite_details(update: Update, context: CallbackContext) -> int:
    ...
    try: days = int(match.group(1)); bots_needed = int(match.group(2)); assert days > 0 and bots_needed > 0
...
async def process_admin_extend_code(update: Update, context: CallbackContext) -> int:
    ...
    await _send_or_edit_message(update, context, get_text(user_id, 'admin_extend_prompt_days', lang=lang, code=code, end_date=end_date_str)); return STATE_WAITING_FOR_EXTEND_DAYS
...
async def process_admin_extend_days(update: Update, context: CallbackContext) -> int:
    ...
    if db.extend_subscription(code, new_end_ts): new_end_date_str = format_dt(new_end_ts); db.log_event_db("Subscription Extended", f"Code: {code}, Added: {days_to_add} days", user_id=user_id, client_id=client.get('user_id')); await _send_or_edit_message(update, context, get_text(user_id, 'admin_extend_success', lang=lang, code=code, days=days_to_add, new_end_date=new_end_date_str))
...
async def process_admin_add_bots_code(update: Update, context: CallbackContext) -> int:
    ...
    context.user_data[CTX_ADD_BOTS_CODE] = code; current_bots = db.get_client_bots(client.get('user_id')) if client.get('user_id') else [b['phone_number'] for b in db.get_all_userbots(assigned_status=True) if b['assigned_client'] == code]
    current_count = len(current_bots)
    await _send_or_edit_message(update, context, get_text(user_id, 'admin_assignbots_prompt_count', lang=lang, code=code, current_count=current_count)); return STATE_WAITING_FOR_ADD_USERBOTS_COUNT
...
async def process_admin_add_bots_count(update: Update, context: CallbackContext) -> int:
    ...
    if len(available_bots) < count_to_add: await _send_or_edit_message(update, context, get_text(user_id, 'admin_assignbots_no_bots_available', lang=lang, needed=count_to_add, available=len(available_bots))); clear_conversation_data(context); return ConversationHandler.END
    success, message = db.assign_userbots_to_client(code, available_bots)
    if success: client_user_id = db.find_client_by_code(code)['user_id'] if db.find_client_by_code(code) else None; db.log_event_db("Userbots Assigned", f"Code: {code}, Count: {len(available_bots)}, Bots: {','.join(available_bots)}", user_id=user_id, client_id=client_user_id); assigned_count_match = re.search(r"Successfully assigned (\d+) userbots", message)
    if assigned_count_match and int(assigned_count_match.group(1)) == len(available_bots): final_message = get_text(user_id, 'admin_assignbots_success', lang=lang, count=len(available_bots), code=code)
    elif assigned_count_match: assigned_count = int(assigned_count_match.group(1)); final_message = get_text(user_id, 'admin_assignbots_partial_success', lang=lang, assigned_count=assigned_count, requested_count=len(available_bots), code=code)
    else: final_message = message; await _send_or_edit_message(update, context, final_message);
    for phone in available_bots: asyncio.create_task(telethon_api.get_userbot_runtime_info(phone))
    else: db.log_event_db("Bot Assign Failed", f"Code: {code}, Reason: {message}", user_id=user_id); fail_message = get_text(user_id, 'admin_assignbots_failed', lang=lang, code=code) + f"\nError: {message}"; await _send_or_edit_message(update, context, fail_message)
    clear_conversation_data(context); return ConversationHandler.END

# --- Folder Management --- (Keep async) ...
async def client_folder_menu(update: Update, context: CallbackContext) -> int:
    user_id, lang = get_user_id_and_lang(update, context); clear_conversation_data(context); lang = context.user_data.get(CTX_LANG, 'en'); folders = db.get_folders_by_user(user_id); text = get_text(user_id, 'folder_menu_title', lang=lang); keyboard = []
    ...
    keyboard.append([InlineKeyboardButton(get_text(user_id, 'button_back', lang=lang), callback_data=f"{CALLBACK_CLIENT_PREFIX}back_to_menu")]); markup = InlineKeyboardMarkup(keyboard); await _send_or_edit_message(update, context, text, reply_markup=markup); return ConversationHandler.END
...
async def client_select_folder_to_edit_or_delete(update: Update, context: CallbackContext, action: str) -> int:
    query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context); _, page_data = query.data.split('?', 1); current_page = int(page_data.split('=')[1]); folders = db.get_folders_by_user(user_id)
    if not folders: await query.answer(get_text(user_id, 'folder_no_folders', lang=lang), show_alert=True); return await client_folder_menu(update, context)
    ...
    base_callback = f"{CALLBACK_FOLDER_PREFIX}select_{action}"; pagination_buttons = build_pagination_buttons(base_callback, current_page, total_items, ITEMS_PER_PAGE); keyboard.extend(pagination_buttons)
...
async def client_show_folder_edit_options(update: Update, context: CallbackContext) -> int:
    ...
    if query: await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return await client_folder_menu(update, context)
    folder_name = db.get_folder_name(folder_id)
    if not folder_name:
        if query: await query.answer(get_text(user_id, 'folder_not_found_error', lang=lang), show_alert=True); return await client_folder_menu(update, context)
    ...
    if groups_in_folder:
        for group in groups_in_folder[:10]: link = group['group_link']; name = group['group_name'] or f"ID: {group['group_id']}"; escaped_link = html.escape(link or ""); escaped_name = html.escape(name); text += f"\n- <a href='{escaped_link}'>{escaped_name}</a>" if link else f"\n- {escaped_name}"
        if len(groups_in_folder) > 10: text += f"\n... and {len(groups_in_folder) - 10} more."
...
async def process_folder_links(update: Update, context: CallbackContext) -> int:
    ...
    if resolver_bot_phone:
        try: resolved_data = await telethon_api.resolve_links_info(resolver_bot_phone, raw_links); link_details.update(resolved_data); log.debug(f"Resolved {len(link_details)} links.")
    ...
    for link, res in results.items():
        ...
        status_key = f"folder_results_{res['status']}"; status_text = get_text(user_id, status_key, lang=lang) if status_key in translations.get(lang, {}) else res['status']
        if res['status'] == 'failed' and res['reason']: status_text += f" ({html.escape(str(res['reason']))})"
...
async def client_select_groups_to_remove(update: Update, context: CallbackContext) -> int:
    ...
    for group in groups_page: db_id = group['id']; is_selected = db_id in selected_ids; prefix = "✅ " if is_selected else "➖ "; link_text = group['group_link'] or f"ID: {group['group_id']}"; display_text = group['group_name'] or link_text; button_text = prefix + html.escape(display_text[:40]) + ("..." if len(display_text) > 40 else ""); callback_data = f"{CALLBACK_FOLDER_PREFIX}edit_toggle_remove?id={db_id}&page={current_page}"; keyboard.append([InlineKeyboardButton(button_text, callback_data=callback_data)])
    base_callback = f"{CALLBACK_FOLDER_PREFIX}edit_remove_select"; pagination_buttons = build_pagination_buttons(base_callback, current_page, total_items, ITEMS_PER_PAGE); keyboard.extend(pagination_buttons)
    action_row = [];
    if selected_ids: action_row.append(InlineKeyboardButton(get_text(user_id, 'folder_edit_remove_confirm_title', lang=lang) + f" ({len(selected_ids)})", callback_data=f"{CALLBACK_FOLDER_PREFIX}edit_remove_confirm"))
...
async def client_toggle_group_for_removal(update: Update, context: CallbackContext) -> int:
    ...
    try: parts = query.data.split('?')[1].split('&'); group_db_id = int(parts[0].split('=')[1]); current_page = int(parts[1].split('=')[1])
    except (ValueError, IndexError): log.error(f"Could not parse group ID/page from callback: {query.data}"); await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return STATE_FOLDER_EDIT_REMOVE_SELECT
...
async def client_confirm_remove_selected_groups(update: Update, context: CallbackContext) -> int:
    ...
    if not folder_id or not folder_name or not ids_to_remove: await query.answer(get_text(user_id, 'folder_edit_remove_none_selected', lang=lang), show_alert=True); query.data = f"{CALLBACK_FOLDER_PREFIX}edit_remove_select?page=0"; return await client_select_groups_to_remove(update, context)
...
async def client_confirm_folder_delete(update: Update, context: CallbackContext) -> int:
    ...
    except (ValueError, IndexError): log.error(f"Could not parse folder ID for delete confirm: {query.data}"); await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return await client_folder_menu(update, context)
    folder_name = db.get_folder_name(folder_id)
...
async def client_delete_folder_confirmed(update: Update, context: CallbackContext) -> int:
    query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context)
    try: _, params = query.data.split('?', 1); folder_id = int(params.split('=')[1])
    except (ValueError, IndexError): log.error(f"Could not parse folder ID for delete confirmed: {query.data}"); await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return await client_folder_menu(update, context)
    folder_name = db.get_folder_name(folder_id)
...
# --- Join Groups Flow --- (Keep async) ...
async def client_select_bot_generic(update: Update, context: CallbackContext, action_prefix: str, next_state: str, title_key: str) -> int: # Return state
    ...
    keyboard = []; text = get_text(user_id, title_key, lang=lang)
    if action_prefix == CALLBACK_JOIN_PREFIX: keyboard.append([InlineKeyboardButton(get_text(user_id, 'join_select_userbot_all', lang=lang), callback_data=f"{action_prefix}select_all")])
    active_bots_count = 0; all_assigned_bots = []
    for phone in user_bots: bot_db_info = db.find_userbot(phone);
    if bot_db_info: all_assigned_bots.append(bot_db_info);
    if bot_db_info and bot_db_info['status'] == 'active': active_bots_count += 1
    for bot_db_info in all_assigned_bots: phone = bot_db_info['phone_number']; username = bot_db_info['username']; display_name = f"@{username}" if username else phone; status_icon = "🟢" if bot_db_info['status'] == 'active' else "⚪️" if bot_db_info['status'] == 'inactive' else "🔴"; button_text = f"{status_icon} {html.escape(display_name)}"; keyboard.append([InlineKeyboardButton(button_text, callback_data=f"{action_prefix}select_{phone}")])
    if action_prefix == CALLBACK_JOIN_PREFIX and active_bots_count < len(all_assigned_bots) and active_bots_count > 0: keyboard.append([InlineKeyboardButton(get_text(user_id, 'join_select_userbot_active', lang=lang, count=active_bots_count), callback_data=f"{action_prefix}select_active")])
...
async def handle_userbot_selection(update: Update, context: CallbackContext, action_prefix: str, next_state: str) -> int:
    ...
    selected_bots = []
    if selected_option == 'all': selected_bots = db.get_client_bots(user_id);
    ...
    elif selected_option == 'active': all_client_bots = db.get_client_bots(user_id); selected_bots = [p for p in all_client_bots if (b := db.find_userbot(p)) and b['status'] == 'active'];
    if not selected_bots: await query.answer(get_text(user_id, 'join_no_active_bots', lang=lang), show_alert=True); return await client_menu(update, context)
    else: phone = selected_option; bot_info = db.find_userbot(phone);
...
async def process_join_group_links(update: Update, context: CallbackContext) -> int:
    ...
    for i, result_item in enumerate(results_list):
        ...
        for link, (status, detail) in results_dict.items():
             status_key = f"join_results_{status}"; status_text = get_text(user_id, status_key, lang=lang) if status_key in translations.get(lang,{}) else status.replace('_',' ').capitalize()
             if isinstance(detail, dict) and detail.get('reason'): reason_key = f"join_results_reason_{detail['reason']}"; reason_text = get_text(user_id, reason_key, lang=lang, error=detail.get('error', ''), seconds=detail.get('seconds', ''));
             if reason_text == reason_key: reason_text = html.escape(str(detail.get('reason'))); status_text = get_text(user_id, 'join_results_failed', lang=lang, reason=reason_text)
             elif status == 'flood_wait' and isinstance(detail, dict): status_text = get_text(user_id, 'join_results_flood_wait', lang=lang, seconds=detail.get('seconds', '?'))
    ...
    if len(all_results_text) > 4096:
        ...
        for i, part in enumerate(parts): part_markup = markup if i == len(parts) - 1 else None; await context.bot.send_message(user_id, part, parse_mode=ParseMode.HTML, reply_markup=part_markup, disable_web_page_preview=True);
...
async def client_show_stats(update: Update, context: CallbackContext) -> int:
    ...
    await _send_or_edit_message(update, context, text, reply_markup=markup); return ConversationHandler.END

# --- Task Setup --- (Keep async) ...
async def task_show_settings_menu(update: Update, context: CallbackContext) -> int:
    ...
    if not phone: log.error(f"Task setup called without phone in context for user {user_id}");
    if query and hasattr(query,'answer'): await query.answer(get_text(user_id, 'session_expired', lang=lang), show_alert=True); return await client_menu(update, context)
    ...
    if not task_settings: task_settings = {}; context.user_data[CTX_TASK_SETTINGS] = task_settings
    elif CTX_TASK_SETTINGS not in context.user_data: context.user_data[CTX_TASK_SETTINGS] = dict(task_settings)
    current_settings = context.user_data.get(CTX_TASK_SETTINGS, {})
    status = current_settings.get('status', 'inactive'); status_icon_key = f'task_status_icon_{status}'; status_icon = get_text(user_id, status_icon_key, lang=lang) if status_icon_key in translations.get(lang,{}) else ("🟢" if status == 'active' else "⚪️"); status_text = get_text(user_id, f'task_status_{status}', lang=lang)
    ...
    else: interval_str = get_text(user_id, 'task_value_not_set', lang=lang)
    ...
    elif current_settings.get('folder_id'): folder_name = db.get_folder_name(current_settings['folder_id']);
    if folder_name: target_str = get_text(user_id, 'task_value_folder', lang=lang, name=html.escape(folder_name))
    else: target_str = get_text(user_id, 'task_value_folder', lang=lang, name="ID: " + str(current_settings.get('folder_id','?'))) + " (Deleted?)"
    last_run_str = format_dt(current_settings.get('last_run')); last_error_raw = current_settings.get('last_error'); last_error = html.escape(last_error_raw[:100]) if last_error_raw else get_text(user_id, 'task_value_not_set', lang=lang)
...
async def process_task_link(update: Update, context: CallbackContext, link_type: str) -> int:
    ...
    link_verified = False # Flag to track if link check was successful
    try:
        ...
        if not accessible:
            ...
            # Stay in the same state if not accessible
            return STATE_WAITING_FOR_PRIMARY_MESSAGE_LINK if link_type == 'primary' else STATE_WAITING_FOR_FALLBACK_MESSAGE_LINK
        else:
            # Link is accessible, set flag to proceed
            log.info(f"User {user_id} link {link_text} verified successfully by bot {phone}.")
            link_verified = True
            # Don't return here, proceed to store the link outside the try block
            pass # Continue execution after the try...except block
         ...
         # Stay in the same state if link check fails due to an unexpected error
         return STATE_WAITING_FOR_PRIMARY_MESSAGE_LINK if link_type == 'primary' else STATE_WAITING_FOR_FALLBACK_MESSAGE_LINK

    # --- Store the link (only if link_verified is True) ---
    if link_verified:
        if link_type == 'primary':
            ...
            # Go back to menu instead of asking fallback for simplicity now
        ...
        # This part should technically not be reached if errors return a state above,
...
async def process_task_start_time(update: Update, context: CallbackContext) -> int:
    ...
    except (ValueError, TypeError, AssertionError): await _send_or_edit_message(update, context, get_text(user_id, 'task_error_invalid_time', lang=lang)); return STATE_WAITING_FOR_START_TIME
    now_local = datetime.now(LITHUANIA_TZ); input_time = datetime.strptime(f"{hour:02}:{minute:02}", "%H:%M").time()
    ...
    task_settings['start_time'] = start_timestamp; log.info(f"User {user_id} set task start time: {time_str} LT -> {start_timestamp} UTC")
...
async def process_interval_callback(update: Update, context: CallbackContext) -> int:
    ...
    try: interval_minutes = int(query.data.split(CALLBACK_INTERVAL_PREFIX)[1]); assert interval_minutes > 0
...
async def task_select_folder_for_target(update: Update, context: CallbackContext) -> int:
    ...
    base_callback = f"{CALLBACK_TASK_PREFIX}select_folder_target"; pagination_buttons = build_pagination_buttons(base_callback, current_page, total_items, ITEMS_PER_PAGE); keyboard.extend(pagination_buttons)
...
async def task_set_target(update: Update, context: CallbackContext, target_type: str) -> int:
    ...
    if target_type == 'all': task_settings['send_to_all_groups'] = 1; task_settings['folder_id'] = None; log.info(f"User {user_id} set task target to all groups.")
        ...
        try: _, params = query.data.split('?', 1); folder_id = int(params.split('=')[1])
        except (ValueError, IndexError): log.error(f"Could not parse folder ID from target callback: {query.data}"); await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return STATE_TASK_SETUP
        folder_name = db.get_folder_name(folder_id)
    ...
    else: log.error(f"Invalid target_type '{target_type}' in task_set_target"); return STATE_TASK_SETUP
    return await task_show_settings_menu(update, context)
...
async def task_toggle_status(update: Update, context: CallbackContext) -> int:
    ...
    task_settings['status'] = new_status; log.info(f"User {user_id} toggled task status to {new_status}")
...
# --- Admin Userbot List/Remove/Log Views --- (Keep async) ...
async def admin_list_userbots(update: Update, context: CallbackContext) -> int: # Return state
    ...
    keyboard = []; base_callback = f"{CALLBACK_ADMIN_PREFIX}list_bots"; pagination_buttons = build_pagination_buttons(base_callback, current_page, total_items, ITEMS_PER_PAGE); keyboard.extend(pagination_buttons)
...
async def admin_select_userbot_to_remove(update: Update, context: CallbackContext) -> int: # Return state
    ...
    base_callback = f"{CALLBACK_ADMIN_PREFIX}remove_bot_select"; pagination_buttons = build_pagination_buttons(base_callback, current_page, total_items, ITEMS_PER_PAGE); keyboard.extend(pagination_buttons)
    keyboard.append([InlineKeyboardButton(get_text(user_id, 'button_back', lang=lang), callback_data=f"{CALLBACK_ADMIN_PREFIX}back_to_menu")]); markup = InlineKeyboardMarkup(keyboard)
    await _send_or_edit_message(update, context, text, reply_markup=markup); return ConversationHandler.END
...
async def admin_confirm_remove_userbot(update: Update, context: CallbackContext) -> int: # Return state
     ...
     except IndexError: log.error(f"Could not parse phone from remove confirm callback: {query.data}"); await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return await admin_command(update, context)
     bot_info = db.find_userbot(phone_to_remove)
     if not bot_info: await query.answer(get_text(user_id, 'admin_userbot_not_found', lang=lang), show_alert=True); return await admin_command(update, context)
...
async def admin_remove_userbot_confirmed(update: Update, context: CallbackContext) -> int: # Return state
    ...
    except IndexError: log.error(f"Could not parse phone from remove confirmed callback: {query.data}"); await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True); return await admin_command(update, context)
    ...
    if db.remove_userbot(phone_to_remove): db.log_event_db("Userbot Removed", f"Phone: {phone_to_remove}", user_id=user_id, userbot_phone=phone_to_remove); await _send_or_edit_message(update, context, get_text(user_id, 'admin_userbot_remove_success', lang=lang, display_name=display_name))
...
async def admin_view_subscriptions(update: Update, context: CallbackContext) -> int: # Return state
    ...
    for sub in subs_page:
        ...
        user_link = f"ID: `{client_user_id}`" # Default value
        # *** CORRECTED INDENTATION AND LOGIC ***
        if client_user_id:
             try:
...
async def admin_view_system_logs(update: Update, context: CallbackContext) -> int: # Return state
    ...
    for log_entry in logs: ts = log_entry['timestamp']; event = log_entry['event']; log_user_id = log_entry['user_id']; log_bot_phone = log_entry['userbot_phone']; details = log_entry['details']; time_str = format_dt(ts)
    ...
    bot_str = html.escape(log_bot_phone) if log_bot_phone else get_text(user_id, 'admin_logs_bot_none', lang=lang); details_str = html.escape(details[:100]) if details else ""
...
async def handle_folder_callback(update: Update, context: CallbackContext) -> str | int | None:
    query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context); data = query.data; action = data.split(CALLBACK_FOLDER_PREFIX)[1].split('?')[0]
    ...
    else: log.warning(f"Unhandled FOLDER CB: Action='{action}', Data='{data}'"); await query.answer("Action not recognized.", show_alert=True)
    return None

async def handle_task_callback(update: Update, context: CallbackContext) -> str | int | None:
    query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context); data = query.data; action = data.split(CALLBACK_TASK_PREFIX)[1].split('?')[0]
    ...
    else: log.warning(f"Unhandled TASK CB: Action='{action}', Data='{data}'"); await query.answer("Action not recognized.", show_alert=True)
    return None

async def handle_join_callback(update: Update, context: CallbackContext) -> str | int | None:
    query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context); data = query.data; log.debug(f"Join Callback Route: Data='{data}'")
    ...
    else: log.warning(f"Unhandled JOIN CB: Data='{data}'"); await query.answer("Action not recognized.", show_alert=True)
    return None

async def handle_language_callback(update: Update, context: CallbackContext) -> str | int | None:
    query = update.callback_query; data = query.data; log.debug(f"Language Callback Route: Data='{data}'")
    if data.startswith(CALLBACK_LANG_PREFIX): return await set_language_handler(update, context)
    else: log.warning(f"Unhandled LANG CB: Data='{data}'"); await query.answer("Action not recognized.", show_alert=True)
    return None

async def handle_interval_callback(update: Update, context: CallbackContext) -> str | int | None:
     query = update.callback_query; data = query.data; log.debug(f"Interval Callback Route: Data='{data}'")
     if data.startswith(CALLBACK_INTERVAL_PREFIX): return await process_interval_callback(update, context)
     else: log.warning(f"Unhandled INTERVAL CB: Data='{data}'"); await query.answer("Action not recognized.", show_alert=True)
     return None
...
async def main_callback_handler(update: Update, context: CallbackContext) -> str | int | None:
    ...
    if not query or not data: log.warning("main_callback_handler received update without query or data."); return None
    log.info(f"CB Route: User={user_id}, Data='{data}'"); next_state = None
    try:
        ...
        else: log.warning(f"Unhandled CB prefix: User={user_id}, Data='{data}'"); await query.answer("Unknown button pressed.", show_alert=True); next_state = ConversationHandler.END
        # Default answer if handler finished without error and didn't explicitly answer
        if query and hasattr(query, 'answered') and not query.answered and CALLBACK_GENERIC_PREFIX+"noop" not in data:
             try: await query.answer()
             except Exception: pass
    except Exception as e:
        log.error(f"Error processing callback data '{data}' for user {user_id}: {e}", exc_info=True)
        try:
             if query and hasattr(query, 'answered') and not query.answered: await query.answer(get_text(user_id, 'error_generic', lang=lang), show_alert=True)
        except Exception: pass
        await _send_or_edit_message(update, context, get_text(user_id, 'error_generic', lang=lang)); clear_conversation_data(context); next_state = ConversationHandler.END
    return next_state


# --- Conversation Handler Definition ---
# Using async def for all handlers now. PTB v13 might warn but should work.
main_conversation = ConversationHandler(
    ...
    states={
        ...
        STATE_WAITING_FOR_GROUP_LINKS: [MessageHandler(Filters.text & ~Filters.command & Filters.chat_type.private, process_join_group_links)], # Note: Also used by folder add link processing
