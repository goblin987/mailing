...
# (Handlers like process_invitation_code, process_admin_phone, etc. follow)
# ... Make sure all these handlers use `await send_or_edit_message(...)` ...
...
async def client_show_folder_edit_options(update: Update, context: CallbackContext) -> int:
    ...
    # We might enter this via message handler (after adding links) or callback
    if query: await query.answer()
    else: # If called via message handler (e.g., after process_folder_links)
        # We assume update is a Message update here
        pass # No query to answer
    ...
    if len(all_results_text) > 4096:
        ...
        for i, part_text in enumerate(parts):
            ...
            try:
                ...
                log.error(f"Error sending split join results part {i+1}: {send_e}"); await context.bot.send_message(user_id, get_text(user_id, 'error_generic', lang_override=lang)); break
    else:
...
# --- Task Setup ---
# ... (task handlers implementation as before, ensure use of `await send_or_edit_message`) ...
...
# --- Admin Handlers ---
# ... (Implementations as before, ensure use of `await send_or_edit_message`) ...
...
# Keep these signatures if they are needed for specific routing logic or as entry points not covered by main_callback_handler.
# For many simple cases, the main_callback_handler routing based on prefix is sufficient.

async def admin_task_menu(update: Update, context: CallbackContext) -> int:
    # Called via callback 'admin_manage_tasks' handled by main_callback_handler
    # which calls this function.
    query = update.callback_query
    if query: await query.answer() # Answer here as main_callback_handler might not
    user_id, lang = get_user_id_and_lang(update, context)
    keyboard = [
...
async def admin_select_task_bot(update: Update, context: CallbackContext) -> int:
    ...
    return STATE_ADMIN_TASK_MESSAGE # Expect message handler after CB selection
...
async def admin_toggle_task_status(update: Update, context: CallbackContext) -> int:
    ...
    if db.toggle_admin_task_status(task_id):
        # Use send_message for confirmation as send_or_edit might replace the menu
