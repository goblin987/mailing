...
async def cancel_command(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        client = db.find_client_by_user_id(user_id)
        if client: return await client_menu(update, context)
...
async def client_menu(update: Update, context: CallbackContext) -> int:
    ...
    client_info = db.find_client_by_user_id(user_id)
    if not client_info:
        await send_or_edit_message(update, context, get_text(user_id, 'unauthorized', lang_override=lang))
        return ConversationHandler.END

    now_ts = int(datetime.now(UTC_TZ).timestamp())
    if client_info['subscription_end'] < now_ts:
        await send_or_edit_message(update, context, get_text(user_id, 'subscription_expired', lang_override=lang))
        return ConversationHandler.END
...
async def client_ask_select_language(update: Update, context: CallbackContext) -> int:
    ...
    if query: await query.answer() # Answer callback if that's how we got here
    user_id, lang = get_user_id_and_lang(update, context)
    ...
    back_cb = f"{CALLBACK_CLIENT_PREFIX}back_to_menu" if db.find_client_by_user_id(user_id) else f"{CALLBACK_ADMIN_PREFIX}back_to_menu" if is_admin(user_id) else None
    if back_cb:
        buttons.append([InlineKeyboardButton(get_text(user_id, 'button_back', lang_override=lang), callback_data=back_cb)])

    markup = InlineKeyboardMarkup(buttons)
    await send_or_edit_message(update, context, get_text(user_id, 'select_language', lang_override=lang), reply_markup=markup)
    # This is just displaying the menu, waiting for CB
    return STATE_WAITING_FOR_LANGUAGE # State for CB handler
...
async def set_language_handler(update: Update, context: CallbackContext) -> int:
    ...
    if lang_code not in language_names:
        ...
        await send_or_edit_message(update, context, get_text(user_id, 'error_invalid_input', lang_override=current_lang_for_cb_answer))
        return ConversationHandler.END
    ...
    if db.set_user_language(user_id, lang_code):
         ...
         keyboard = []
         if back_cb:
             keyboard.append([InlineKeyboardButton(get_text(user_id, 'button_main_menu', lang_override=lang_code), callback_data=back_cb)])
         markup = InlineKeyboardMarkup(keyboard) if keyboard else None
         await send_or_edit_message(update, context, success_text, reply_markup=markup)
    else:
        log.error(f"set_language_handler: Failed to set language in DB for user {user_id} to {lang_code}")
        await send_or_edit_message(update, context, get_text(user_id, 'language_set_error', lang_override=current_lang_for_cb_answer))
...
async def process_folder_links(update: Update, context: CallbackContext) -> int:
    ...
    for link in raw_links:
        ...
        if resolved and not resolved.get('error'):
            ...
            status_code = 'failed'; reason = get_text(user_id, 'folder_resolve_error', lang_override=lang) + " (No resolver or not in results)"; failed_count += 1
    ...
    for link, res in results.items():
        ...
        status_text_template = get_text(user_id, status_key, lang_override=lang)
...
async def client_select_groups_to_remove(update: Update, context: CallbackContext) -> int:
    ...
    if not groups:
        await send_or_edit_message(update, context, get_text(user_id, 'folder_edit_no_groups', lang_override=lang))
        return await client_show_folder_edit_options(update, context)
...
async def process_join_group_links(update: Update, context: CallbackContext) -> int:
    ...
    for i, result_item in enumerate(results_list_from_gather):
        ...
        for link, (status_code, detail_dict_or_str) in results_dict_for_bot.items():
             ...
             status_text = get_text(user_id, status_key_from_join, lang_override=lang)
    ...
    if len(all_results_text) > 4096:
        ...
        for i, part_text in enumerate(parts):
            ...
            try:
                ...
                await context.bot.send_message(user_id, get_text(user_id, 'error_generic', lang_override=lang))
                break
    else:
...
# --- Task Setup ---
# ... (task handlers: task_show_settings_menu, task_prompt_set_link, process_task_link, etc.) ...
# Ensure all these use `await send_or_edit_message` and correct context keys/states from config.py
...
# --- (Other task handlers: prompt_set_link, process_task_link, prompt_start_time, etc.) ---
# --- Ensure they use `await send_or_edit_message` and correct imports/constants ---
# --- (Code for these handlers omitted for brevity but should follow the pattern above) ---
...
async def process_task_link(update: Update, context: CallbackContext, link_type: str) -> int:
    ...
    await send_or_edit_message(update, context, get_text(user_id, 'task_verifying_link', lang_override=lang))
    link_verified = False
    try:
...
async def task_select_interval(update: Update, context: CallbackContext) -> int:
    query = update.callback_query; await query.answer()
    user_id, lang = get_user_id_and_lang(update, context)
    intervals = [5, 10, 15, 30, 60, 120, 180, 240, 360, 720, 1440]
    keyboard = []; row = []
...
async def task_set_target(update: Update, context: CallbackContext, target_type_from_cb: str) -> int:
    ...
    if target_type_from_cb == 'all':
        ...
        log.error(f"Invalid target_type '{target_type_from_cb}' in task_set_target."); return STATE_TASK_SETUP
    return await task_show_settings_menu(update, context)
...
# --- Admin Handlers ---
# ... (admin handlers: list_userbots, select_userbot_to_remove, confirm_remove, etc.) ...
# Ensure all these use `await send_or_edit_message` and correct imports/constants
...
async def admin_select_userbot_to_remove(update: Update, context: CallbackContext) -> int:
    ...
    all_bots = db.get_all_userbots()
    if not all_bots:
        text = get_text(user_id, 'admin_userbot_no_bots_to_remove', lang_override=lang); markup = InlineKeyboardMarkup([[InlineKeyboardButton(get_text(user_id, 'button_back', lang_override=lang), callback_data=f"{CALLBACK_ADMIN_PREFIX}back_to_menu")]]); await send_or_edit_message(update, context, text, reply_markup=markup); return ConversationHandler.END
    total_items = len(all_bots); start_index = current_page * ITEMS_PER_PAGE; end_index = start_index + ITEMS_PER_PAGE; bots_page = all_bots[start_index:end_index]
    text = get_text(user_id, 'admin_userbot_select_remove', lang_override=lang); keyboard = []
    for bot in bots_page:
        phone = bot['phone_number']; username = bot['username']; display_name = f"@{username}" if username else phone; button_text = f"🗑️ {html.escape(display_name)}"; keyboard.append([InlineKeyboardButton(button_text, callback_data=f"{CALLBACK_ADMIN_PREFIX}remove_bot_confirm_prompt_{phone}")])
...
async def admin_view_subscriptions(update: Update, context: CallbackContext) -> int:
    ...
    if not subs:
        text = get_text(user_id, 'admin_subs_none', lang_override=lang); markup = InlineKeyboardMarkup([[InlineKeyboardButton(get_text(user_id, 'button_back', lang_override=lang), callback_data=f"{CALLBACK_ADMIN_PREFIX}back_to_menu")]]); await send_or_edit_message(update, context, text, reply_markup=markup); return ConversationHandler.END
...
async def admin_view_system_logs(update: Update, context: CallbackContext) -> int:
    ...
    if not logs_from_db:
        text = get_text(user_id, 'admin_logs_none', lang_override=lang); markup = InlineKeyboardMarkup([[InlineKeyboardButton(get_text(user_id, 'button_back', lang_override=lang), callback_data=f"{CALLBACK_ADMIN_PREFIX}back_to_menu")]]); await send_or_edit_message(update, context, text, reply_markup=markup); return ConversationHandler.END
...
main_conversation = ConversationHandler(
    ...
    states={
        ...
        STATE_WAITING_FOR_GROUP_LINKS: [MessageHandler(Filters.text & ~Filters.command, process_folder_links)], # Also handles join links
        ...
        STATE_WAITING_FOR_LANGUAGE: [CallbackQueryHandler(main_callback_handler)], # Language selection via CB
    },
    fallbacks=[
        CommandHandler('cancel', cancel_command),
...
# --- Admin Task Management Handlers (kept for routing from callbacks) ---
# (These should mostly delegate to the main_callback_handler now or handle specific CBs)

async def admin_task_menu(update: Update, context: CallbackContext) -> int:
    # Called via callback 'admin_manage_tasks'
    query = update.callback_query
    if query: await query.answer()
    user_id, lang = get_user_id_and_lang(update, context)
    keyboard = [
...
async def admin_select_task_bot(update: Update, context: CallbackContext) -> int:
    ...
    return STATE_ADMIN_TASK_MESSAGE # Expect msg handler after CB selection
...
async def admin_toggle_task_status(update: Update, context: CallbackContext) -> int:
    ...
    if db.toggle_admin_task_status(task_id):
        # Temporarily send a success message before showing options again
        ...
        query.data = f"{CALLBACK_ADMIN_PREFIX}task_options_{task_id}" # Modify query data to re-route
        return await admin_task_options(update, context) # Show options again
    else:
