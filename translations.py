...
def error_handler(update: object, context: CallbackContext) -> None:
    ...
    # Always clear conversation data after logging the error
...
def client_menu(update: Update, context: CallbackContext) -> int:
    ...
    user_id, lang = get_user_id_and_lang(update, context)
    # Answer callback query if update is from a callback
    if update.callback_query:
        try:
            update.callback_query.answer()
        except BadRequest as e:
            log.debug(f"Ignoring error answering client_menu callback query: {e}")
...
def process_admin_api_hash(update: Update, context: CallbackContext) -> str | int:
    ...
    # Basic check for typical hash format/length - adjust if needed
...
def handle_client_callback(update: Update, context: CallbackContext) -> str | int | None:
    ...
    elif action == "back_to_menu": return client_menu(update, context) # Answered in client_menu
...
def handle_folder_callback(update: Update, context: CallbackContext) -> str | int | None:
     # Stub - Needs implementation to route folder actions
     query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context)
...
def handle_task_callback(update: Update, context: CallbackContext) -> str | int | None:
     # Stub - Needs implementation to route task setup actions
     query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context)
...
def handle_language_callback(update: Update, context: CallbackContext) -> str | int | None:
     # Implemented - routes to set_language_handler
...
def handle_generic_callback(update: Update, context: CallbackContext) -> str | int | None:
     # Implemented - handles cancel/confirm_no for now
     query = update.callback_query; user_id, lang = get_user_id_and_lang(update, context); data = query.data
...
def main_callback_handler(update: Update, context: CallbackContext) -> str | int | None:
    ...
    # Answer callback here to avoid forgetting in sub-handlers (can be answered again)
    try: query.answer()
    ...
    elif data.startswith(CALLBACK_FOLDER_PREFIX): return handle_folder_callback(update, context) # Needs Impl
    elif data.startswith(CALLBACK_TASK_PREFIX): return handle_task_callback(update, context) # Needs Impl
    elif data.startswith(CALLBACK_JOIN_PREFIX): return handle_join_callback(update, context) # Needs Impl
    elif data.startswith(CALLBACK_LANG_PREFIX): return handle_language_callback(update, context) # Impl
    elif data.startswith(CALLBACK_INTERVAL_PREFIX): return handle_interval_callback(update, context) # Needs Impl
    elif data.startswith(CALLBACK_GENERIC_PREFIX): return handle_generic_callback(update, context) # Impl
    else: log.warning(f"Unhandled CB prefix: User={user_id}, Data='{data}'"); return None # Indicate unhandled
...
main_conversation = ConversationHandler(
    entry_points=[
        ...
        # Handles button presses outside a specific state (e.g., main menu buttons)
        CallbackQueryHandler(main_callback_handler)
    ],
    states={
        # Define states and the handlers that process messages/callbacks within them
        ...
        # Other Admin states (use stubs for now)
        ...
        # Client states (many use callbacks routed by main_callback_handler)
    ...
    fallbacks=[
        ...
        MessageHandler(Filters.all & Filters.chat_type.private, conversation_fallback) # General fallback
    ],
    allow_reentry=True
)
