                 ...
                 if connected:
                      ...
                      try:
                          ...
                          db.update_userbot_status(phone_number, 'active', username=me.username if me else None)
                           ...
                           # Decide if status should be error or stay connecting/initializing
                           db.update_userbot_status(phone_number, 'error', last_error=f"get_me failed: {get_me_err}")
                 else:
...
async def start_authentication_flow(phone, api_id, api_hash):
    ...
    try:
        ...
        auth_result_data = {'client': temp_client, 'loop': temp_loop, 'phone_code_hash': code_request_info.phone_code_hash}
         ...
         try:
              if not temp_client: # Should not happen, but safety check
                   temp_client = await _create_telethon_client_instance(session_file, api_id, api_hash, temp_loop)
              ...
              auth_result_status = 'error'; auth_result_data = f"Failed to get password details: {pwd_err}"
    except FloodWaitError as e:
        log.warning(f"Flood wait during code request for {phone}: {e.seconds}s"); auth_result_status = 'error'; auth_result_data = f"Flood wait: Try again in {e.seconds} seconds."
    except (PhoneNumberInvalidError, ApiIdInvalidError, ApiIdPublishedFloodError) as e:
         log.error(f"Config/Phone error code request {phone}: {e}"); auth_result_status = 'error'; auth_result_data = f"Invalid config/phone: {e}"
    except AuthKeyError:
         log.error(f"AuthKeyError during initial code request for {phone}."); auth_result_status = 'error'; auth_result_data = "Auth key error. Please try again."
    except (TelethonConnectionError, asyncio.TimeoutError, OSError) as e:
         log.error(f"Connection issue code request {phone}: {e}"); auth_result_status = 'error'; auth_result_data = f"Connection failed: {e}"
    except Exception as e:
        log.exception(f"Unexpected error auth start {phone}: {e}"); auth_result_status = 'error'; auth_result_data = f"Unexpected error: {e}"
...
async def complete_authentication_flow(auth_data, code=None, password=None):
    ...
    phone = "UnknownPhone" # Will be updated on success
    final_status = 'error'
    final_data = "Unknown auth failure"

    if not temp_client or not temp_loop: return 'error', "Invalid auth data (client/loop missing)."

    me_object = None
    try:
        if code:
            phone_code_hash = auth_data.get('phone_code_hash')
            if not phone_code_hash: return 'error', "Internal Error: Missing phone_code_hash."
        ...
        else: return 'error', "No code or password provided."
        ...
        if me_object and await temp_client.is_user_authorized():
            ...
            if db_add_ok:
                final_status = 'success'; final_data = {'phone': phone, 'username': username}
                log.info(f"Userbot {phone} info saved to DB.")
                get_userbot_runtime_info(phone) # Initialize persistent runtime
    ...
    except (TelethonConnectionError, asyncio.TimeoutError, OSError) as e: final_status = 'error'; final_data = f"Connection failed: {e}"
    ...
    if final_status == 'error' and isinstance(final_data, str):
        final_data = {'error_message': final_data} # Standardize error data format
...
async def join_groups_batch(phone, urls):
        ...
        async with lock:
            try:
                ...
                for i, url in enumerate(urls):
                    ...
                    entity = None # Reset entity for each URL
                    try:
                        ...
                        if link_type == "private_join":
                            log.info(f"{bot_display}: Joining via invite: {identifier}")
                            ...
                            else: raise InviteHashInvalidError("Import request empty.")
                        elif link_type == "public":
                            log.info(f"{bot_display}: Resolving public: {identifier}")
                            entity = await client.get_entity(identifier)
                            if not isinstance(entity, (Channel, TelethonChat)): raise ValueError("Link points to user.")
                            log.info(f"{bot_display}: Joining public: {getattr(entity,'title','N/A')}")
                    ...
                    # UserBannedInChatError REMOVED (Check specific case below)
                    except ChatWriteForbiddenError: # Can indicate banned or other write restrictions
                         status, detail = "failed", {"reason": "banned_or_restricted"}
                         log.warning(f"{bot_display}: ChatWriteForbidden joining {url}")
                        ...
                        wait_time = min(e.seconds + random.uniform(1, 3), 90); log.warning(f"{bot_display}: Flood joining {url} ({e.seconds}s). Wait {wait_time:.1f}s."); await asyncio.sleep(wait_time); status, detail = "flood_wait", {"seconds": e.seconds}
                    except ValueError as e: status, detail = "failed", {"reason": f"invalid_link_or_resolve: {e}"}
                    except (AuthKeyError, TelethonConnectionError, asyncio.TimeoutError, OSError) as e: log.error(f"{bot_display}: Conn error joining {url}: {e}. Abort."); db.update_userbot_status(phone, 'error', f"Conn Err: {e}"); raise ConnectionError(f"Conn fail: {e}")
                    except UserDeactivatedBanError as e: log.critical(f"{bot_display}: BANNED joining {url}: {e}. Abort."); db.update_userbot_status(phone, 'error', "Account Banned"); raise e
                    ...
                    log.info(f"{bot_display}: Result for {url}: {status} ({(time.monotonic()-start_time):.2f}s)")
                    if i < len(urls) - 1: # Add delay unless it's the last one
    ...
    try:
        timeout = (len(urls) * (CLIENT_TIMEOUT / 2 + 5)) + 30
        error_info, results_dict = future.result(timeout=timeout)
        return error_info, results_dict
    except asyncio.TimeoutError:
        log.error(f"Timeout join batch {phone}."); final_results = locals().get('results_dict', {}); [final_results.setdefault(url, ("failed", {"reason": "batch_timeout"})) for url in urls if url not in final_results]; return {"error": "Batch join timeout."}, final_results
    except Exception as e:
        log.exception(f"Error join batch future {phone}: {e}"); final_results = locals().get('results_dict', {}); [final_results.setdefault(url, ("failed", {"reason": f"internal_error: {e}"})) for url in urls if url not in final_results]; error_info = {"error": str(e)} if not locals().get('error_info') else locals()['error_info']; return error_info, final_results
            ...
            try:
                ...
                async for dialog in client.iter_dialogs(limit=500):
                    ...
                    if dialog.is_group or dialog.is_channel:
                        entity = dialog.entity; link = None; id_part = None
                        username = getattr(entity, 'username', None)
...
async def get_message_entity_by_link(client, link):
    ...
    try:
        ...
        # Optional: Verify message ID existence (disabled for performance)
...
async def _forward_single_message(client, target_peer, source_chat_entity, message_id, fallback_chat_entity=None, fallback_message_id=None):
    ...
    except (AuthKeyError, TelethonConnectionError, asyncio.TimeoutError, OSError) as e: err_reason = f"Conn/Sess Error: {type(e).__name__}"; log.error(f"[{phone}] -> {err_reason} Target={target_id}: {e}"); raise ConnectionError(err_reason) from e # Critical
    ...
    try:
        async with lock:
            ...
            if task_info['send_to_all_groups']:
                ...
                if err: raise ConnectionError(f"Task fail get targets: {err.get('error')}")
            ...
            if target_count == 0: log.warning(f"[{phone}] No target groups for task. Finish.")
                ...
                processed = set(); delay = max(1.0, min(5.0, 30 / target_count)) # Dynamic delay
                for target_id in target_ids:
                    ...
                    try:
                        ...
                        elif fwd_err and final_err is None and "Flood Wait" not in fwd_err and "Conn/Sess Error" not in fwd_err: final_err = f"Target {target_id}: {fwd_err}" # Store first non-critical err
                    ...
                    await asyncio.sleep(delay + random.uniform(0, 0.5)) # Wait before next
...
def shutdown_telethon():
    ...
    for phone in phones: stop_userbot_runtime(phone) # Handles disconnect/stop/join
